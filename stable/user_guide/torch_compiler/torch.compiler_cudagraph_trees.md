...
- Arbitrary Control Flow is not possible (However, control flow expressed via `torch.cond()` can be captured in a CUDA Graph. See [Data Dependent Control Flow](../../notes/cuda.html#graph-data-dependent-control-flow).)
