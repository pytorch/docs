...
[`torch.export.export()`](api_reference.html#torch.export.export).
...
specifically with [`torch.fx`](../../../fx.html#module-torch.fx) and its related toolings. Thus it will stop
describing contents present in [`torch.fx`](../../../fx.html#module-torch.fx) documentation and paper.

## What is Export IR

Export IR is a graph-based intermediate representation IR of PyTorch programs.
Export IR is realized on top of [`torch.fx.Graph`](../../../fx.html#torch.fx.Graph). In other words, **all
...
The top-level Export IR construct is an [`torch.export.ExportedProgram`](api_reference.html#torch.export.ExportedProgram)
class. It bundles the computational graph of a PyTorch model (which is usually a
[`torch.nn.Module`](../../../generated/torch.nn.Module.html#torch.nn.Module)) with the parameters or weights that this model
consumes.

Some notable attributes of the [`torch.export.ExportedProgram`](api_reference.html#torch.export.ExportedProgram) class are:

- `graph_module` ([`torch.fx.GraphModule`](../../../fx.html#torch.fx.GraphModule)): Data structure containing
...
In practice, Export IR's graph is realized as [`torch.fx.Graph`](../../../fx.html#torch.fx.Graph) Python class.
...
Python using the [`torch.fx.Node`](../../../fx.html#torch.fx.Node) class. Edges between nodes are
...
value in the list is an [`torch.fx.Node`](../../../fx.html#torch.fx.Node), then it will be especially
...
[`torch.nn.Module`](../../../generated/torch.nn.Module.html#torch.nn.Module) from which the node came, if it was from a
[`torch.nn.Module`](../../../generated/torch.nn.Module.html#torch.nn.Module) call. For example, if a node containing the `addmm`
op called from a [`torch.nn.Linear`](../../../generated/torch.nn.Linear.html#torch.nn.Linear) module inside of a
[`torch.nn.Sequential`](../../../generated/torch.nn.Sequential.html#torch.nn.Sequential) module, the `nn_module_stack` would look
something like:
...
[`torch.nn.Module`](../../../generated/torch.nn.Module.html#torch.nn.Module) class this node was called from before decomposition.
For example, a node containing the `addmm` op from a
[`torch.nn.Linear`](../../../generated/torch.nn.Linear.html#torch.nn.Linear) module call would contain [`torch.nn.Linear`](../../../generated/torch.nn.Linear.html#torch.nn.Linear) in
their `source_fn`, and a node containing the `addmm` op from a
`torch.nn.functional.Linear` module call would contain
...
This has the exact semantics as in [`torch.fx`](../../../fx.html#module-torch.fx). `args` represents the node
to be returned.
...
[`torch.fx.GraphModule`](../../../fx.html#torch.fx.GraphModule). Unlike a vanilla FX graph from
[`torch.fx.symbolic_trace()`](../../../fx.html#torch.fx.symbolic_trace) in which `get_attr` nodes are used to read
attributes such as parameters and buffers from the top-level
[`torch.fx.GraphModule`](../../../fx.html#torch.fx.GraphModule), parameters and buffers are passed in as
inputs to the graph module, and stored in the top-level
[`torch.export.ExportedProgram`](api_reference.html#torch.export.ExportedProgram).
...
| Type | Definition |
| --- | --- |
| Tensor | [`torch.Tensor`](../../../tensors.html#torch.Tensor) |
...
| bool | Python bool |
| str | Python string |
| ScalarType | [`torch.dtype`](../../../tensor_attributes.html#torch.dtype) |
| Layout | [`torch.layout`](../../../tensor_attributes.html#torch.layout) |
| MemoryFormat | [`torch.memory_format`](../../../tensor_attributes.html#torch.memory_format) |
| Device | [`torch.device`](../../../tensor_attributes.html#torch.device) |
