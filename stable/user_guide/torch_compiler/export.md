...
[`torch.export.export()`](export/api_reference.html#torch.export.export) takes a [`torch.nn.Module`](../../generated/torch.nn.Module.html#torch.nn.Module) and produces a traced graph
...
[`torch.compile()`](../../generated/torch.compile.html#torch.compile) also utilizes the same PT2 stack as `torch.export`, but
is slightly different:

- **JIT vs. AOT**: [`torch.compile()`](../../generated/torch.compile.html#torch.compile) is a JIT compiler whereas
which is not intended to be used to produce compiled artifacts outside of
deployment.
- **Partial vs. Full Graph Capture**: When [`torch.compile()`](../../generated/torch.compile.html#torch.compile) runs into an
...
- **Usability tradeoff**: Since [`torch.compile()`](../../generated/torch.compile.html#torch.compile) is able to fallback to the
...
Compared to [`torch.fx.symbolic_trace()`](../../fx.html#torch.fx.symbolic_trace), `torch.export` traces using
...
level). Note that users can still use [`torch.fx.symbolic_trace()`](../../fx.html#torch.fx.symbolic_trace) as a
...
Compared to `torch.jit.trace()`, `torch.export` is sound:
...
[`torch.nn.Module`](../../generated/torch.nn.Module.html#torch.nn.Module) and sample inputs, and
captures the computation graph into an [`torch.export.ExportedProgram`](export/api_reference.html#torch.export.ExportedProgram). An
example:
...
- The [`torch.fx.Graph`](../../fx.html#torch.fx.Graph) contains the computation graph of the original
...
the result of [`torch.fx.symbolic_trace()`](../../fx.html#torch.fx.symbolic_trace).
...
[`torch.export.export()`](export/api_reference.html#torch.export.export) through the `dynamic_shapes` argument.
...
utility `torch.export.ShapesCollection`, where instead of specifying the
...
`torch.export.AdditionalInputs` in place of `dynamic_shapes`. You can
...
To save the `ExportedProgram`, users can use the [`torch.export.save()`](export/api_reference.html#torch.export.save) and
[`torch.export.load()`](export/api_reference.html#torch.export.load) APIs. The resulting file is a zipfile with a specific
...
`ExportedProgram.run_decompositions()` API. This method automatically:
...
[non-strict export](export/programming_model.html#non-strict-export) through changing the `strict` flag
to `strict=False`.
...
- [torch.export API Reference](export/api_reference.html)
- [torch.export Programming Model](export/programming_model.html)
- [torch.export IR Specification](export/ir_spec.html)
- [PT2 Archive Spec](export/pt2_archive.html)
- [Draft Export](export/draft_export.html)
- [Joint with descriptors](export/joint_with_descriptors.html)
- [Control Flow Operators](../../higher_order_ops/index.html)
