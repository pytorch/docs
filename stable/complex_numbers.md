...
Complex numbers are numbers that can be expressed in the form a+bja + bja+bj, where a and b are real numbers,
and *j* is called the imaginary unit, which satisfies the equation j2=−1j^2 = -1j2=−1. Complex numbers frequently occur in mathematics and
...
complex tensors (e.g., [`torch.mv()`](generated/torch.mv.html#torch.mv), [`torch.matmul()`](generated/torch.matmul.html#torch.matmul)) are likely to be faster and more memory efficient
...
All factory functions apart from [`torch.linspace()`](generated/torch.linspace.html#torch.linspace), [`torch.logspace()`](generated/torch.logspace.html#torch.logspace), and [`torch.arange()`](generated/torch.arange.html#torch.arange) are
supported for complex tensors.

## Transition from the old representation

Users who currently worked around the lack of complex tensors with real tensors of shape (...,2)(..., 2)(...,2)
can easily switch to using the complex tensors in their code using [`torch.view_as_complex()`](generated/torch.view_as_complex.html#torch.view_as_complex)
and [`torch.view_as_real()`](generated/torch.view_as_real.html#torch.view_as_real). Note that these functions don't perform any copy and return a
...
The real and imaginary values of a complex tensor can be accessed using the `real` and
`imag`.
...
The angle and absolute values of a complex tensor can be computed using [`torch.angle()`](generated/torch.angle.html#torch.angle) and
[`torch.abs()`](generated/torch.abs.html#torch.abs).
...
Many linear algebra operations, like [`torch.matmul()`](generated/torch.matmul.html#torch.matmul), [`torch.linalg.svd()`](generated/torch.linalg.svd.html#torch.linalg.svd), [`torch.linalg.solve()`](generated/torch.linalg.solve.html#torch.linalg.solve) etc., support complex numbers.
...
check out the note [Autograd for Complex Numbers](notes/autograd.html#complex-autograd-doc).

## Optimizers

Semantically, we define stepping through a PyTorch optimizer with complex parameters as being equivalent to stepping
through the same optimizer on the [`torch.view_as_real()`](generated/torch.view_as_real.html#torch.view_as_real) equivalent of the complex params. More concretely:
...
[`torch.view_as_real()`](generated/torch.view_as_real.html#torch.view_as_real) equivalent will convert a complex tensor to a real tensor with shape (...,2)(..., 2)(...,2),
