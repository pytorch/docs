...
torch.median(*input*) → [Tensor](../tensors.html#torch.Tensor)
...
compute the mean of both medians, use [`torch.quantile()`](torch.quantile.html#torch.quantile) with `q=0.5` instead.
...
`input`, use [`torch.quantile()`](torch.quantile.html#torch.quantile) with `q=0.5` instead.
