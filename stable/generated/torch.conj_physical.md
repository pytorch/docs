...
torch.conj_physical(*input*, ***, *out=None*) → [Tensor](../tensors.html#torch.Tensor)
...
In the future, `torch.conj_physical()` may return a non-writeable view for an `input` of
non-complex dtype. It's recommended that programs not modify the tensor returned by `torch.conj_physical()`
