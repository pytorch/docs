...
torch.nn.functional.embedding_bag(*input*, *weight*, *offsets=None*, *max_norm=None*, *norm_type=2*, *scale_grad_by_freq=False*, *mode='mean'*, *sparse=False*, *per_sample_weights=None*, *include_last_offset=False*, *padding_idx=None*)[[source]](https://github.com/pytorch/pytorch/blob/v2.12.0/torch/nn/functional.py#L2570)

Compute sums, means or maxes of bags of embeddings.

Calculation is done without instantiating the intermediate embeddings.
See [`torch.nn.EmbeddingBag`](torch.nn.EmbeddingBag.html#torch.nn.EmbeddingBag) for more details.
...
- **weight** ([*Tensor*](../tensors.html#torch.Tensor)) - The embedding matrix with number of rows equal to the maximum possible index + 1,
