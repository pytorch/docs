...
torch.frombuffer(*buffer*, ***, *dtype*, *count=-1*, *offset=0*, *requires_grad=False*) → [Tensor](../tensors.html#torch.Tensor)

Creates a 1-dimensional [`Tensor`](../tensors.html#torch.Tensor) from an object that implements
the Python buffer protocol.

Skips the first `offset` bytes in the buffer, and interprets the rest of
the raw bytes as a 1-dimensional tensor of type [`dtype`](../tensor_attributes.html#torch.dtype) with `count`
elements.
...
(in bytes) of [`dtype`](../tensor_attributes.html#torch.dtype).
...
[`dtype`](../tensor_attributes.html#torch.dtype).
...
This function does not try to infer the [`dtype`](../tensor_attributes.html#torch.dtype) (hence, it is not
optional). Passing a different [`dtype`](../tensor_attributes.html#torch.dtype) than its source may result
