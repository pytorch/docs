...
Inverse short time Fourier Transform. This is expected to be the inverse of [`stft()`](torch.stft.html#torch.stft).
...
∑t=−∞∞∣w∣2[n−t×hop_length]=0\sum_{t=-\infty}^{\infty} |w|^2[n-t\times hop\_length] \cancel{=} 0∑t=−∞∞​∣w∣2[n−t×hop_length]=​0.

Since [`stft()`](torch.stft.html#torch.stft) discards elements at the end of the signal if they do not fit in a frame,
...
The input tensor. Expected to be in the format of [`stft()`](torch.stft.html#torch.stft),
