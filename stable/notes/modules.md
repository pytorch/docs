...
To get started, let's look at a simpler, custom version of PyTorch's [`Linear`](../generated/torch.nn.Linear.html#torch.nn.Linear) module.
...
All modules should subclass [`Module`](../generated/torch.nn.Module.html#torch.nn.Module) for composability with other modules.
...
transformation. Because each of these is defined as a [`Parameter`](../generated/torch.nn.parameter.Parameter.html#torch.nn.parameter.Parameter), they are
*registered* for the module and will automatically be tracked and returned from calls
to [`parameters()`](../generated/torch.nn.Module.html#torch.nn.Module.parameters). Parameters can be
...
[`parameters()`](../generated/torch.nn.Module.html#torch.nn.Module.parameters) or [`named_parameters()`](../generated/torch.nn.Module.html#torch.nn.Module.named_parameters),
...
The simplest way to do this is using the [`Sequential`](../generated/torch.nn.Sequential.html#torch.nn.Sequential) module. It allows us to chain together
multiple modules:
...
Note that [`Sequential`](../generated/torch.nn.Sequential.html#torch.nn.Sequential) automatically feeds the output of the first `MyLinear` module as input
into the [`ReLU`](../generated/torch.nn.ReLU.html#torch.nn.ReLU), and the output of that as input into the second `MyLinear` module. As
...
children of a module can be iterated through via a call to [`children()`](../generated/torch.nn.Module.html#torch.nn.Module.children) or
[`named_children()`](../generated/torch.nn.Module.html#torch.nn.Module.named_children):
...
To go deeper than just the immediate children, [`modules()`](../generated/torch.nn.Module.html#torch.nn.Module.modules) and
[`named_modules()`](../generated/torch.nn.Module.html#torch.nn.Module.named_modules) *recursively* iterate through a module and its child modules:
...
The [`ModuleList`](../generated/torch.nn.ModuleList.html#torch.nn.ModuleList) and [`ModuleDict`](../generated/torch.nn.ModuleDict.html#torch.nn.ModuleDict) modules are useful here; they
...
This means that calls to [`parameters()`](../generated/torch.nn.Module.html#torch.nn.Module.parameters) and [`named_parameters()`](../generated/torch.nn.Module.html#torch.nn.Module.named_parameters) will
...
[`to()`](../generated/torch.nn.Module.html#torch.nn.Module.to):
...
using the [`apply()`](../generated/torch.nn.Module.html#torch.nn.Module.apply) function. For example, to apply custom initialization to parameters
...
provides a large library of performant modules within the [`torch.nn`](../nn.html#module-torch.nn) namespace that perform common neural
...
Optimizers from [`torch.optim`](../optim.html#module-torch.optim):
...
to its absolute value by employing [`torch.abs()`](../generated/torch.abs.html#torch.abs) as a loss function. While this is not a very interesting task, the
...
training mode and can be switched between training and evaluation modes using [`train()`](../generated/torch.nn.Module.html#torch.nn.Module.train) and
[`eval()`](../generated/torch.nn.Module.html#torch.nn.Module.eval). They can behave differently depending on which mode they are in. For example, the
`BatchNorm` module maintains a running mean and variance during training that are not updated
...
This snippet shows how to use [`register_buffer()`](../generated/torch.nn.Module.html#torch.nn.Module.register_buffer) to accomplish this:
...
Buffers of a module can be iterated over using [`buffers()`](../generated/torch.nn.Module.html#torch.nn.Module.buffers) or
[`named_buffers()`](../generated/torch.nn.Module.html#torch.nn.Module.named_buffers).
...
By default, parameters and floating-point buffers for modules provided by [`torch.nn`](../nn.html#module-torch.nn) are initialized during
...
[`torch.nn`](../nn.html#module-torch.nn) modules follow:
...
[`register_forward_pre_hook()`](../generated/torch.nn.Module.html#torch.nn.Module.register_forward_pre_hook) and [`register_forward_hook()`](../generated/torch.nn.Module.html#torch.nn.Module.register_forward_hook).
...
[`register_module_forward_pre_hook()`](../generated/torch.nn.modules.module.register_module_forward_pre_hook.html#torch.nn.modules.module.register_module_forward_pre_hook) and
[`register_module_forward_hook()`](../generated/torch.nn.modules.module.register_module_forward_hook.html#torch.nn.modules.module.register_module_forward_hook) functions.
- **Backward hooks** are called during the backward pass. They can be installed with
[`register_full_backward_pre_hook()`](../generated/torch.nn.Module.html#torch.nn.Module.register_full_backward_pre_hook) and [`register_full_backward_hook()`](../generated/torch.nn.Module.html#torch.nn.Module.register_full_backward_hook).
These hooks will be called when the backward for this Module has been computed.
[`register_full_backward_pre_hook()`](../generated/torch.nn.Module.html#torch.nn.Module.register_full_backward_pre_hook) will allow the user to access the gradients for outputs
while [`register_full_backward_hook()`](../generated/torch.nn.Module.html#torch.nn.Module.register_full_backward_hook) will allow the user to access the gradients
both the inputs and outputs. Alternatively, they can be installed globally for all modules with
[`register_module_full_backward_hook()`](../generated/torch.nn.modules.module.register_module_full_backward_hook.html#torch.nn.modules.module.register_module_full_backward_hook) and
[`register_module_full_backward_pre_hook()`](../generated/torch.nn.modules.module.register_module_full_backward_pre_hook.html#torch.nn.modules.module.register_module_full_backward_pre_hook).
