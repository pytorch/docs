...
`torch.svd()` is deprecated in favor of [`torch.linalg.svd()`](torch.linalg.svd.html#torch.linalg.svd)
...
Differences with [`torch.linalg.svd()`](torch.linalg.svd.html#torch.linalg.svd):

- `some` is the opposite of
[`torch.linalg.svd()`](torch.linalg.svd.html#torch.linalg.svd)'s `full_matrices`. Note that
default value for both is True, so the default behavior is
effectively the opposite.
- `torch.svd()` returns V, whereas [`torch.linalg.svd()`](torch.linalg.svd.html#torch.linalg.svd) returns
Vh, that is, VHV^{\text{H}}VH.
- If `compute_uv` is False, `torch.svd()` returns zero-filled
tensors for U and Vh, whereas [`torch.linalg.svd()`](torch.linalg.svd.html#torch.linalg.svd) returns
empty tensors.
...
as U and V may be multiplied by an arbitrary phase factor eiϕe^{i \phi}eiϕ on every column.
