...
You can use [`torch.manual_seed()`](../generated/torch.manual_seed.html#torch.manual_seed) to seed the RNG for all devices (both
CPU and CUDA):
...
[`torch.svd_lowrank()`](../generated/torch.svd_lowrank.html#torch.svd_lowrank) does this, for instance. Consequently, calling it
multiple times back-to-back with the same input arguments may give different
results. However, as long as [`torch.manual_seed()`](../generated/torch.manual_seed.html#torch.manual_seed) is set to a constant
...
random numbers by setting [`torch.manual_seed()`](../generated/torch.manual_seed.html#torch.manual_seed) to the same value between
subsequent calls.
...
[`torch.use_deterministic_algorithms()`](../generated/torch.use_deterministic_algorithms.html#torch.use_deterministic_algorithms) lets you configure PyTorch to use
...
Please check the documentation for [`torch.use_deterministic_algorithms()`](../generated/torch.use_deterministic_algorithms.html#torch.use_deterministic_algorithms)
...
When [`torch.bmm()`](../generated/torch.bmm.html#torch.bmm) is called with sparse-dense CUDA tensors it typically uses a
...
See [`torch.nn.RNN()`](../generated/torch.nn.RNN.html#torch.nn.RNN) and [`torch.nn.LSTM()`](../generated/torch.nn.LSTM.html#torch.nn.LSTM) for details and workarounds.

### Filling uninitialized memory

Operations like [`torch.empty()`](../generated/torch.empty.html#torch.empty) and [`torch.Tensor.resize_()`](../generated/torch.Tensor.resize_.html#torch.Tensor.resize_) can return
...
[`torch.utils.deterministic.fill_uninitialized_memory`](../deterministic.html#torch.utils.deterministic.fill_uninitialized_memory) is set to `True`
