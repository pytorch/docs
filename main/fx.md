...
or [`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module) instances), and return values. More information
...
[`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module) instance that holds a `Graph` as well as a
...
[examples](https://github.com/pytorch/examples/tree/master/fx)
repository.
...
Your transform will take in a [`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module), acquire a `Graph`
from it, do some modifications, and return a new
[`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module). You should think of the [`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module) that your FX
transform returns as identical to a regular [`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module) - you can pass it to another
FX transform, or you can run it. Ensuring that the inputs and outputs of your FX transform are a
[`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module) will allow for composability.

Note

It is also possible to modify an existing `GraphModule` instead of
...
Given that you've passed in a [`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module) that has been traced into a
...
All three of these concepts are represented with `Node` instances.
...
[`torch.add()`](generated/torch.add.html#torch.add) calls with [`torch.mul()`](generated/torch.mul.html#torch.mul) calls.
...
We can also do more involved `Graph` rewrites, such as
...
`Graph`\s. It allows you to specify a `pattern` and `replacement` function
...
Another way of manipulating `Graph`\s is by reusing the `Proxy`
...
PyTorch code and invoke that code with `Proxy` objects as arguments.
...
In addition to avoiding explicit graph manipulation, using `Proxy`\s
...
maintainability of the rules. Note that while calling `Proxy` we also
...
the call to `Proxy` does not create multiple instances of a graph
tracer which can lead to unexpected runtime errors. We recommend this method
of using `Proxy` especially when the underlying operators can not be
safely assumed to be unary.

A worked example of using `Proxy`\s for `Graph` manipulation
can be found
[here](https://github.com/pytorch/examples/blob/master/fx/proxy_based_graph_creation.py).

### The Interpreter Pattern

A useful code organizational pattern in FX is to loop over all the `Node`\s
...
via retracing with `Proxy`\s. For example, suppose we want to run a
`GraphModule` and record the [`torch.Tensor`](tensors.html#torch.Tensor) shape and dtype
...
point [`torch.Tensor`](tensors.html#torch.Tensor) instances, checking for equivalence between
the results of two [`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module) is not as straightforward
...
details). We can use [`torch.allclose()`](generated/torch.allclose.html#torch.allclose) instead, which will give
...
`GraphModule.to_folder()` is a method in `GraphModule` that allows
...
[`torch.nn.Module`](generated/torch.nn.Module.html#torch.nn.Module) or function) to record operations. It is
...
torch.fx.symbolic_trace(*root*, *concrete_args=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/_symbolic_trace.py#L1389)
...
- **root** (*Union**[*[*torch.nn.Module*](generated/torch.nn.Module.html#torch.nn.Module)*,**Callable**]*) - Module or function to be traced and converted
into a Graph representation.
- **concrete_args** (*Optional**[**Dict**[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*,**any**]**]*) - Inputs to be partially specialized
...
torch.fx.wrap(*fn_or_name: _F*) → _F[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/_symbolic_trace.py#L1320)

torch.fx.wrap(*fn_or_name: [str](https://docs.python.org/3/builtins/stdtypes.html#str)*) → [str](https://docs.python.org/3/builtins/stdtypes.html#str)
...
**fn_or_name** (*Union**[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*,**Callable**]*) - The function or name of the global function to insert into the
...
*class*torch.fx.GraphModule(**args*, ***kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph_module.py#L518)
...
__init__(*root*, *graph*, *class_name='GraphModule'*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph_module.py#L552)

Construct a GraphModule.

Parameters:

- **root** (*Union**[*[*torch.nn.Module*](generated/torch.nn.Module.html#torch.nn.Module)*,**Dict**[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*,**Any**]*) - `root` can either be an nn.Module instance or a Dict mapping strings to any attribute type.
...
- **class_name** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - `name` denotes the name of this GraphModule for debugging purposes. If it's unset, all
...
add_submodule(*target*, *m*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph_module.py#L775)
...
- **target** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - The fully-qualified string name of the new submodule
(See example in `nn.Module.get_submodule` for how to
specify a fully-qualified string.)
- **m** ([*Module*](generated/torch.nn.Module.html#torch.nn.Module)) - The submodule itself; the actual object we want to
...
*property*code*: [str](https://docs.python.org/3/builtins/stdtypes.html#str)*

Return the Python code generated from the `Graph` underlying this
`GraphModule`.

delete_all_unused_submodules()[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph_module.py#L857)
...
delete_submodule(*target*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph_module.py#L815)
...
print_readable(*print_output=True*, *include_stride=False*, *include_device=False*, *colored=False*, ***, *fast_sympy_print=False*, *expanded_def=False*, *additional_meta=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph_module.py#L1125)

Return the Python code generated for current GraphModule and its children GraphModules.

Parameters:

**additional_meta** ([*list*](https://docs.python.org/3/builtins/stdtypes.html#list)*[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*]**|**None*) - Optional list of meta keys to include in the output.
...
recompile()[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph_module.py#L925)
...
to_folder(*folder*, *module_name='FxModule'*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph_module.py#L690)
...
*class*torch.fx.Graph(*owning_module=None*, *tracer_cls=None*, *tracer_extras=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1398)
...
__init__(*owning_module=None*, *tracer_cls=None*, *tracer_extras=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1447)
...
call_function(*the_function*, *args=None*, *kwargs=None*, *type_expr=None*, *name=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1960)
...
- **kwargs** (*Optional**[**Dict**[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*,**Argument**]**]*) - The keyword arguments to be passed
to the called function
- **type_expr** (*Optional**[**Any**]*) - an optional type annotation representing the
Python type the output of this node will have.
- **name** (*Optional**[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*]*) - The name of the node. If not specified, set to None
...
call_method(*method_name*, *args=None*, *kwargs=None*, *type_expr=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1921)
...
- **method_name** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - The name of the method to apply to the self argument.
...
call_module(*module_name*, *args=None*, *kwargs=None*, *type_expr=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1868)
...
- **module_name** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - The qualified name of the `Module` in the `Module`
...
create_node(*op*, *target*, *args=None*, *kwargs=None*, *name=None*, *type_expr=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1586)
...
- **op** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - the opcode for this Node. One of 'call_function', 'call_method', 'get_attr',
'call_module', 'placeholder', or 'output'. The semantics of these opcodes are
described in the `Graph` docstring.
- **args** (*Optional**[**Tuple**[**Argument**,**...**]**]*) - is a tuple of arguments to this node.
- **kwargs** (*Optional**[**Dict**[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*,**Argument**]**]*) - the kwargs of this Node
- **name** (*Optional**[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*]*) - an optional string name for the `Node`.
...
create_size_node(*tensor_node*, *dim*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2019)
...
create_storage_offset_node(*tensor_node*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2037)
...
create_stride_node(*tensor_node*, *dim*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2028)
...
eliminate_dead_code(*is_impure_node=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2691)
...
- **is_impure_node** (*Optional**[**Callable**[**[**Node**]**,*[*bool*](https://docs.python.org/3/builtins/functions.html#bool)*]**]*) - A function that returns
...
erase_node(*to_erase*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1676)
...
find_nodes(***, *op*, *target=None*, *sort=True*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1499)

Allows for fast query of nodes

Parameters:

- **op** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - the name of the operation
...
- **sort** ([*bool*](https://docs.python.org/3/builtins/functions.html#bool)) - whether to return nodes in the order they appear on
on the graph.
...
get_attr(*qualified_name*, *type_expr=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1800)
...
- **qualified_name** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - the fully-qualified name of the attribute to be retrieved.
...
graph_copy(*g*, *val_map*, *return_output_node=False*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1526)
...
[tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[[tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[Argument, ...] | [*Sequence*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Sequence)[Argument] | [*Mapping*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Mapping)[[str](https://docs.python.org/3/builtins/stdtypes.html#str), Argument] | [slice](https://docs.python.org/3/builtins/functions.html#slice) | [range](https://docs.python.org/3/builtins/stdtypes.html#range) | *Node* | [str](https://docs.python.org/3/builtins/stdtypes.html#str) | [int](https://docs.python.org/3/builtins/functions.html#int) | [float](https://docs.python.org/3/builtins/functions.html#float) | [bool](https://docs.python.org/3/builtins/functions.html#bool) | [complex](https://docs.python.org/3/builtins/functions.html#complex) | [*dtype*](tensor_attributes.html#torch.dtype) | [*Tensor*](tensors.html#torch.Tensor) | [*device*](tensor_attributes.html#torch.device) | [*memory_format*](tensor_attributes.html#torch.memory_format) | [*layout*](tensor_attributes.html#torch.layout) | *OpOverload* | [*SymInt*](torch.html#torch.SymInt) | [*SymBool*](torch.html#torch.SymBool) | [*SymFloat*](torch.html#torch.SymFloat) | None, ...] | [*Sequence*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Sequence)[[tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[Argument, ...] | [*Sequence*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Sequence)[Argument] | [*Mapping*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Mapping)[[str](https://docs.python.org/3/builtins/stdtypes.html#str), Argument] | [slice](https://docs.python.org/3/builtins/functions.html#slice) | [range](https://docs.python.org/3/builtins/stdtypes.html#range) | *Node* | [str](https://docs.python.org/3/builtins/stdtypes.html#str) | [int](https://docs.python.org/3/builtins/functions.html#int) | [float](https://docs.python.org/3/builtins/functions.html#float) | [bool](https://docs.python.org/3/builtins/functions.html#bool) | [complex](https://docs.python.org/3/builtins/functions.html#complex) | [*dtype*](tensor_attributes.html#torch.dtype) | [*Tensor*](tensors.html#torch.Tensor) | [*device*](tensor_attributes.html#torch.device) | [*memory_format*](tensor_attributes.html#torch.memory_format) | [*layout*](tensor_attributes.html#torch.layout) | *OpOverload* | [*SymInt*](torch.html#torch.SymInt) | [*SymBool*](torch.html#torch.SymBool) | [*SymFloat*](torch.html#torch.SymFloat) | None] | [*Mapping*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Mapping)[[str](https://docs.python.org/3/builtins/stdtypes.html#str), [tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[Argument, ...] | [*Sequence*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Sequence)[Argument] | [*Mapping*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Mapping)[[str](https://docs.python.org/3/builtins/stdtypes.html#str), Argument] | [slice](https://docs.python.org/3/builtins/functions.html#slice) | [range](https://docs.python.org/3/builtins/stdtypes.html#range) | *Node* | [str](https://docs.python.org/3/builtins/stdtypes.html#str) | [int](https://docs.python.org/3/builtins/functions.html#int) | [float](https://docs.python.org/3/builtins/functions.html#float) | [bool](https://docs.python.org/3/builtins/functions.html#bool) | [complex](https://docs.python.org/3/builtins/functions.html#complex) | [*dtype*](tensor_attributes.html#torch.dtype) | [*Tensor*](tensors.html#torch.Tensor) | [*device*](tensor_attributes.html#torch.device) | [*memory_format*](tensor_attributes.html#torch.memory_format) | [*layout*](tensor_attributes.html#torch.layout) | *OpOverload* | [*SymInt*](torch.html#torch.SymInt) | [*SymBool*](torch.html#torch.SymBool) | [*SymFloat*](torch.html#torch.SymFloat) | None] | [slice](https://docs.python.org/3/builtins/functions.html#slice) | [range](https://docs.python.org/3/builtins/stdtypes.html#range) | *Node* | [str](https://docs.python.org/3/builtins/stdtypes.html#str) | [int](https://docs.python.org/3/builtins/functions.html#int) | [float](https://docs.python.org/3/builtins/functions.html#float) | [bool](https://docs.python.org/3/builtins/functions.html#bool) | [complex](https://docs.python.org/3/builtins/functions.html#complex) | [*dtype*](tensor_attributes.html#torch.dtype) | [*Tensor*](tensors.html#torch.Tensor) | [*device*](tensor_attributes.html#torch.device) | [*memory_format*](tensor_attributes.html#torch.memory_format) | [*layout*](tensor_attributes.html#torch.layout) | *OpOverload* | [*SymInt*](torch.html#torch.SymInt) | [*SymBool*](torch.html#torch.SymBool) | [*SymFloat*](torch.html#torch.SymFloat) | None

Note

Backwards-compatibility for this API is guaranteed.

inserting_after(*n=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1742)
...
inserting_before(*n=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1717)
...
lint()[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2611)
...
materialize_symint(*value*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2382)
...
*Node* | [int](https://docs.python.org/3/builtins/functions.html#int)

materialize_symints(*values*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2111)
...
[list](https://docs.python.org/3/builtins/stdtypes.html#list)[*Node* | [int](https://docs.python.org/3/builtins/functions.html#int)]

node_copy(*node*, *arg_transform=<function Graph.<lambda>>*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2387)
...
on_generate_code(*make_transformer*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2784)
...
output(*result*, *type_expr=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2424)
...
output_node()[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1492)
...
placeholder(*name*, *type_expr=None*, *default_value*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1767)
...
- **name** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - A name for the input value. This corresponds to the name
...
print_tabular()[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2589)
...
process_inputs(**args*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1665)
...
process_outputs(*out*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L1672)
...
python_code(*root_module*, ***, *verbose=False*, *include_stride=False*, *include_device=False*, *colored=False*, *expanded_def=False*, *record_func=False*, *additional_meta=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2462)

Turn this `Graph` into valid Python code.

Parameters:

**root_module** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - The name of the root module on which to look-up
...
set_codegen(*codegen*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/graph.py#L2780)

Warning

This API is experimental and is *NOT* backward-compatible.

*class*torch.fx.Node(*graph*, *name*, *op*, *target*, *args*, *kwargs*, *return_type=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/node.py#L259)
...
*property*all_input_nodes*: [list](https://docs.python.org/3/builtins/stdtypes.html#list)[Node]*
...
append(*x*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/node.py#L442)
...
format_node(*placeholder_names=None*, *maybe_return_typename=None*, ***, *include_tensor_metadata=False*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/node.py#L617)
...
- **placeholder_names** ([*list*](https://docs.python.org/3/builtins/stdtypes.html#list)*[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*]**|**None*) - A list that will store formatted strings
representing the placeholders in the generated
`forward` function. Internal use only.
- **maybe_return_typename** ([*list*](https://docs.python.org/3/builtins/stdtypes.html#list)*[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*]**|**None*) - A single-element list that will store
a formatted string representing the output of the
generated `forward` function. Internal use only.
- **include_tensor_metadata** ([*bool*](https://docs.python.org/3/builtins/functions.html#bool)) - Whether to include tensor metadata
...
insert_arg(*idx*, *arg*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/node.py#L529)

Insert a positional argument to the argument list with given index.

Parameters:

- **idx** ([*int*](https://docs.python.org/3/builtins/functions.html#int)) - The index of the element in `self.args` to be inserted before.
...
is_impure(*impure_random=True*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/node.py#L758)
...
**impure_random** ([*bool*](https://docs.python.org/3/builtins/functions.html#bool)) - Whether to treat rand op as impure.
...
*property*kwargs*: [dict](https://docs.python.org/3/builtins/stdtypes.html#dict)[[str](https://docs.python.org/3/builtins/stdtypes.html#str), [tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[[tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[Argument, ...] | [Sequence](https://docs.python.org/3/library/collections.abc.html#collections.abc.Sequence)[Argument] | [Mapping](https://docs.python.org/3/library/collections.abc.html#collections.abc.Mapping)[[str](https://docs.python.org/3/builtins/stdtypes.html#str), Argument] | [slice](https://docs.python.org/3/builtins/functions.html#slice) | [range](https://docs.python.org/3/builtins/stdtypes.html#range) | Node | [str](https://docs.python.org/3/builtins/stdtypes.html#str) | [int](https://docs.python.org/3/builtins/functions.html#int) | [float](https://docs.python.org/3/builtins/functions.html#float) | [bool](https://docs.python.org/3/builtins/functions.html#bool) | [complex](https://docs.python.org/3/builtins/functions.html#complex) | [dtype](tensor_attributes.html#torch.dtype) | [Tensor](tensors.html#torch.Tensor) | [device](tensor_attributes.html#torch.device) | [memory_format](tensor_attributes.html#torch.memory_format) | [layout](tensor_attributes.html#torch.layout) | OpOverload | [SymInt](torch.html#torch.SymInt) | [SymBool](torch.html#torch.SymBool) | [SymFloat](torch.html#torch.SymFloat) | [None](https://docs.python.org/3/builtins/constants.html#None), ...] | [Sequence](https://docs.python.org/3/library/collections.abc.html#collections.abc.Sequence)[[tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[Argument, ...] | [Sequence](https://docs.python.org/3/library/collections.abc.html#collections.abc.Sequence)[Argument] | [Mapping](https://docs.python.org/3/library/collections.abc.html#collections.abc.Mapping)[[str](https://docs.python.org/3/builtins/stdtypes.html#str), Argument] | [slice](https://docs.python.org/3/builtins/functions.html#slice) | [range](https://docs.python.org/3/builtins/stdtypes.html#range) | Node | [str](https://docs.python.org/3/builtins/stdtypes.html#str) | [int](https://docs.python.org/3/builtins/functions.html#int) | [float](https://docs.python.org/3/builtins/functions.html#float) | [bool](https://docs.python.org/3/builtins/functions.html#bool) | [complex](https://docs.python.org/3/builtins/functions.html#complex) | [dtype](tensor_attributes.html#torch.dtype) | [Tensor](tensors.html#torch.Tensor) | [device](tensor_attributes.html#torch.device) | [memory_format](tensor_attributes.html#torch.memory_format) | [layout](tensor_attributes.html#torch.layout) | OpOverload | [SymInt](torch.html#torch.SymInt) | [SymBool](torch.html#torch.SymBool) | [SymFloat](torch.html#torch.SymFloat) | [None](https://docs.python.org/3/builtins/constants.html#None)] | [Mapping](https://docs.python.org/3/library/collections.abc.html#collections.abc.Mapping)[[str](https://docs.python.org/3/builtins/stdtypes.html#str), [tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[Argument, ...] | [Sequence](https://docs.python.org/3/library/collections.abc.html#collections.abc.Sequence)[Argument] | [Mapping](https://docs.python.org/3/library/collections.abc.html#collections.abc.Mapping)[[str](https://docs.python.org/3/builtins/stdtypes.html#str), Argument] | [slice](https://docs.python.org/3/builtins/functions.html#slice) | [range](https://docs.python.org/3/builtins/stdtypes.html#range) | Node | [str](https://docs.python.org/3/builtins/stdtypes.html#str) | [int](https://docs.python.org/3/builtins/functions.html#int) | [float](https://docs.python.org/3/builtins/functions.html#float) | [bool](https://docs.python.org/3/builtins/functions.html#bool) | [complex](https://docs.python.org/3/builtins/functions.html#complex) | [dtype](tensor_attributes.html#torch.dtype) | [Tensor](tensors.html#torch.Tensor) | [device](tensor_attributes.html#torch.device) | [memory_format](tensor_attributes.html#torch.memory_format) | [layout](tensor_attributes.html#torch.layout) | OpOverload | [SymInt](torch.html#torch.SymInt) | [SymBool](torch.html#torch.SymBool) | [SymFloat](torch.html#torch.SymFloat) | [None](https://docs.python.org/3/builtins/constants.html#None)] | [slice](https://docs.python.org/3/builtins/functions.html#slice) | [range](https://docs.python.org/3/builtins/stdtypes.html#range) | Node | [str](https://docs.python.org/3/builtins/stdtypes.html#str) | [int](https://docs.python.org/3/builtins/functions.html#int) | [float](https://docs.python.org/3/builtins/functions.html#float) | [bool](https://docs.python.org/3/builtins/functions.html#bool) | [complex](https://docs.python.org/3/builtins/functions.html#complex) | [dtype](tensor_attributes.html#torch.dtype) | [Tensor](tensors.html#torch.Tensor) | [device](tensor_attributes.html#torch.device) | [memory_format](tensor_attributes.html#torch.memory_format) | [layout](tensor_attributes.html#torch.layout) | OpOverload | [SymInt](torch.html#torch.SymInt) | [SymBool](torch.html#torch.SymBool) | [SymFloat](torch.html#torch.SymFloat) | [None](https://docs.python.org/3/builtins/constants.html#None)]*
...
normalized_arguments(*root*, *arg_types=None*, *kwarg_types=None*, *normalize_to_only_use_kwargs=False*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/node.py#L809)
...
- **root** ([*torch.nn.Module*](generated/torch.nn.Module.html#torch.nn.Module)) - Module upon which to resolve module targets.
- **arg_types** (*Optional**[**Tuple**[**Any**]**]*) - Tuple of arg types for the args
- **kwarg_types** (*Optional**[**Dict**[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*,**Any**]**]*) - Dict of arg types for the kwargs
- **normalize_to_only_use_kwargs** ([*bool*](https://docs.python.org/3/builtins/functions.html#bool)) - Whether to normalize to only use kwargs.
...
prepend(*x*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/node.py#L426)
...
replace_all_uses_with(*replace_with*, *delete_user_cb=None*, ***, *propagate_meta=False*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/node.py#L711)
...
- **propagate_meta** ([*bool*](https://docs.python.org/3/builtins/functions.html#bool)) - Whether or not to copy all properties
...
[list](https://docs.python.org/3/builtins/stdtypes.html#list)[*Node*]

Note

Backwards-compatibility for this API is guaranteed.

replace_input_with(*old_input*, *new_input*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/node.py#L867)
...
*property*stack_trace*: [str](https://docs.python.org/3/builtins/stdtypes.html#str) | [None](https://docs.python.org/3/builtins/constants.html#None)*
...
update_arg(*idx*, *arg*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/node.py#L514)
...
- **idx** ([*int*](https://docs.python.org/3/builtins/functions.html#int)) - The index into `self.args` of the element to update
...
update_kwarg(*key*, *arg*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/node.py#L556)
...
- **key** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - The key in `self.kwargs` of the element to update
...
*class*torch.fx.Tracer(*autowrap_modules=(math,)*, *autowrap_functions=()*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/_symbolic_trace.py#L281)
...
call_module(*m*, *forward*, *args*, *kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/_symbolic_trace.py#L558)
...
- **m** ([*Module*](generated/torch.nn.Module.html#torch.nn.Module)) - The module for which a call is being emitted
...
create_arg(*a*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/_symbolic_trace.py#L402)
...
create_args_for_root(*root_fn*, *is_module*, *concrete_args=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/_symbolic_trace.py#L687)
...
[tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[[*Any*](https://docs.python.org/3/library/typing.html#typing.Any), [list](https://docs.python.org/3/builtins/stdtypes.html#list)[[*Any*](https://docs.python.org/3/library/typing.html#typing.Any)]]

create_node(*kind*, *target*, *args*, *kwargs*, *name=None*, *type_expr=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/proxy.py#L214)
...
create_proxy(*kind*, *target*, *args*, *kwargs*, *name=None*, *type_expr=None*, *proxy_factory_fn=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/proxy.py#L339)
...
get_fresh_qualname(*prefix*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/_symbolic_trace.py#L376)
...
getattr(*attr*, *attr_val*, *parameter_proxy_cache*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/_symbolic_trace.py#L618)
...
- **attr** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - The name of the attribute being queried
- **attr_val** (*Any*) - The value of the attribute
- **parameter_proxy_cache** (*Dict**[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*,**Any**]*) - A cache of attr names to proxies
...
is_leaf_module(*m*, *module_qualified_name*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/_symbolic_trace.py#L505)
...
- **m** ([*Module*](generated/torch.nn.Module.html#torch.nn.Module)) - The module being queried about
- **module_qualified_name** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - The path to root of this module. For example,
...
iter(*obj*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/proxy.py#L496)
...
keys(*obj*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/proxy.py#L514)
...
path_of_module(*mod*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/_symbolic_trace.py#L530)
...
**mod** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - The `Module` to retrieve the qualified name for.
...
proxy(*node*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/proxy.py#L335)
...
to_bool(*obj*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/proxy.py#L485)
...
trace(*root*, *concrete_args=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/_symbolic_trace.py#L798)
...
- **root** (*Union**[*[*Module*](generated/torch.nn.Module.html#torch.nn.Module)*,**Callable**]*) - Either a `Module` or a function to be
traced through. Backwards-compatibility for this parameter is
guaranteed.
- **concrete_args** (*Optional**[**Dict**[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*,**any**]**]*) - Concrete arguments that should
...
*class*torch.fx.Proxy(*node*, *tracer=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/proxy.py#L599)
...
*class*torch.fx.Interpreter(*module*, *garbage_collect_values=True*, *graph=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L51)
...
- **module** ([*torch.nn.Module*](generated/torch.nn.Module.html#torch.nn.Module)) - The module to be executed
- **garbage_collect_values** ([*bool*](https://docs.python.org/3/builtins/functions.html#bool)) - Whether to delete values after their last
...
boxed_run(*args_list*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L239)
...
call_function(*target*, *args*, *kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L356)
...
call_method(*target*, *args*, *kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L379)
...
call_module(*target*, *args*, *kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L404)
...
fetch_args_kwargs_from_env(*n*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L472)
...
fetch_attr(*target*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L451)

Fetch an attribute from the `Module` hierarchy of `self.module`.

Parameters:

**target** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - The fully-qualified name of the attribute to fetch
...
get_attr(*target*, *args*, *kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L334)
...
map_nodes_to_values(*args*, *n*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L494)
...
output(*target*, *args*, *kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L430)
...
placeholder(*target*, *args*, *kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L297)
...
run(**args*, *initial_env=None*, *enable_io_processing=True*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L147)
...
the interpreter.
- **enable_io_processing** ([*bool*](https://docs.python.org/3/builtins/functions.html#bool)) - If true, we process the inputs and outputs with graph's process_inputs and
...
run_node(*n*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L273)
...
*class*torch.fx.Transformer(*module*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L517)
...
call_function(*target*, *args*, *kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L640)
...
call_module(*target*, *args*, *kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L630)
...
get_attr(*target*, *args*, *kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L610)
...
placeholder(*target*, *args*, *kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L587)
...
transform()[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/interpreter.py#L647)
...
torch.fx.replace_pattern(*gm*, *pattern*, *replacement*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/subgraph_rewriter.py#L95)
...
- **pattern** ([*Callable*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Callable)*[**[**...**]**,*[*Any*](https://docs.python.org/3/library/typing.html#typing.Any)*]**|**GraphModule*) - The subgraph to match in `gm` for replacement
- **replacement** ([*Callable*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Callable)*[**[**...**]**,*[*Any*](https://docs.python.org/3/library/typing.html#typing.Any)*]**|**GraphModule*) - The subgraph to replace `pattern` with
...
torch.fx.traceback.annotate(*annotation_dict*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/traceback.py#L309)
...
torch.fx.passes.tools_common.stable_topological_sort(*gm*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/tools_common.py#L333)
...
**gm** (*GraphModule*) - The graph module to topologically sort. It is modified in-place.
...
| [`annotate`](generated/torch.fx.annotate.annotate.html#torch.fx.annotate.annotate) | Annotates a Proxy object with a given type. |
| --- | --- |

## torch.fx.node

| [`has_side_effect`](generated/torch.fx.node.has_side_effect.html#torch.fx.node.has_side_effect) | Registers a function to not be dead code eliminated by fx.graph.eliminate_dead_code |
| --- | --- |
| [`map_aggregate`](generated/torch.fx.node.map_aggregate.html#torch.fx.node.map_aggregate) | Apply fn recursively to each object appearing in arg. |
| [`map_arg`](generated/torch.fx.node.map_arg.html#torch.fx.node.map_arg) | Apply fn recursively to each Node appearing in arg. |
...
| [`replace_pattern`](generated/torch.fx.subgraph_rewriter.replace_pattern.html#torch.fx.subgraph_rewriter.replace_pattern) | Matches all possible non-overlapping sets of operators and their data dependencies (`pattern`) in the Graph of a GraphModule (`gm`), then replaces each of these matched subgraphs with another subgraph (`replacement`). |
| --- | --- |
| [`replace_pattern_with_filters`](generated/torch.fx.subgraph_rewriter.replace_pattern_with_filters.html#torch.fx.subgraph_rewriter.replace_pattern_with_filters) | See replace_pattern for documentation. |

## torch.fx.tensor_type

| [`is_consistent`](generated/torch.fx.tensor_type.is_consistent.html#torch.fx.tensor_type.is_consistent) | A binary relation denoted by ~ that determines if t1 is consistent with t2. |
| --- | --- |
| [`is_more_precise`](generated/torch.fx.tensor_type.is_more_precise.html#torch.fx.tensor_type.is_more_precise) | A binary relation denoted by <= that determines if t1 is more precise than t2. |

## torch.fx.passes.backends.cudagraphs

| [`partition_cudagraphs`](generated/torch.fx.passes.backends.cudagraphs.partition_cudagraphs.html#torch.fx.passes.backends.cudagraphs.partition_cudagraphs) | Partition an FX graph into sub-GraphModules that can be validly run under CUDA graphs. |
| --- | --- |

## torch.fx.passes.graph_manipulation

torch.fx.passes.graph_manipulation.get_size_of_all_nodes(*fx_module*, *args=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/graph_manipulation.py#L56)
...
torch.fx.passes.graph_manipulation.get_size_of_node(*fx_module*, *node*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/graph_manipulation.py#L87)
...
| [`pass_result_wrapper`](generated/torch.fx.passes.infra.pass_manager.pass_result_wrapper.html#torch.fx.passes.infra.pass_manager.pass_result_wrapper) | Wrapper for passes which currently do not return a PassResult. |
| --- | --- |
| [`this_before_that_pass_constraint`](generated/torch.fx.passes.infra.pass_manager.this_before_that_pass_constraint.html#torch.fx.passes.infra.pass_manager.this_before_that_pass_constraint) | Defines a partial order ('depends on' function) where `this` must occur before `that`. |

## torch.fx.passes.operator_support

| [`any_chain`](generated/torch.fx.passes.operator_support.any_chain.html#torch.fx.passes.operator_support.any_chain) | Combines a sequence of OperatorSupportBase instances to form a single OperatorSupportBase |
| --- | --- |
| [`chain`](generated/torch.fx.passes.operator_support.chain.html#torch.fx.passes.operator_support.chain) | Combines a sequence of OperatorSupportBase instances to form a single OperatorSupportBase |
| [`create_op_support`](generated/torch.fx.passes.operator_support.create_op_support.html#torch.fx.passes.operator_support.create_op_support) | Wraps a IsNodeSupported function into an OperatorSupportBase instance |

## torch.fx.passes.param_fetch

torch.fx.passes.param_fetch.default_matching(*name*, *target_version*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/param_fetch.py#L18)
...
torch.fx.passes.param_fetch.extract_attrs_for_lowering(*mod*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/param_fetch.py#L58)
...
torch.fx.passes.param_fetch.lift_lowering_attrs_to_nodes(*fx_module*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/param_fetch.py#L85)
...
| [`inplace_wrapper`](generated/torch.fx.passes.pass_manager.inplace_wrapper.html#torch.fx.passes.pass_manager.inplace_wrapper) | Convenience wrapper for passes which modify an object inplace. |
| --- | --- |
| [`log_hook`](generated/torch.fx.passes.pass_manager.log_hook.html#torch.fx.passes.pass_manager.log_hook) | Logs callable output. |
| [`loop_pass`](generated/torch.fx.passes.pass_manager.loop_pass.html#torch.fx.passes.pass_manager.loop_pass) | Convenience wrapper for passes which need to be applied multiple times. |
| [`these_before_those_pass_constraint`](generated/torch.fx.passes.pass_manager.these_before_those_pass_constraint.html#torch.fx.passes.pass_manager.these_before_those_pass_constraint) | Defines a partial order ('depends on' function) where `these` must occur before `those`. |
| [`this_before_that_pass_constraint`](generated/torch.fx.passes.pass_manager.this_before_that_pass_constraint.html#torch.fx.passes.pass_manager.this_before_that_pass_constraint) | Defines a partial order ('depends on' function) where this must occur before that. |

## torch.fx.passes.regional_inductor

| [`regional_inductor`](generated/torch.fx.passes.regional_inductor.regional_inductor.html#torch.fx.passes.regional_inductor.regional_inductor) | Scoops out inductor marked regions and compiles them with inductor. |
| --- | --- |

## torch.fx.passes.reinplace

| [`reinplace`](generated/torch.fx.passes.reinplace.reinplace.html#torch.fx.passes.reinplace.reinplace) | Given an fx.GraphModule, modifies it to perform "reinplacing", mutating the nodes of the graph. |
| --- | --- |

## torch.fx.passes.split_utils

torch.fx.passes.split_utils.split_by_tags(*gm*, *tags*, *return_fqn_mapping=False*, *return_tuple=False*, *GraphModuleCls=<class 'torch.fx.graph_module.GraphModule'>*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/split_utils.py#L76)
...
| [`is_node_output_tensor`](generated/torch.fx.passes.tools_common.is_node_output_tensor.html#torch.fx.passes.tools_common.is_node_output_tensor) | Checks if the node output produces a Tensor or not. |
| --- | --- |

## torch.fx.passes.utils.common

torch.fx.passes.utils.common.lift_subgraph_as_module(*gm*, *subgraph*, *comp_name=''*, *class_name='GraphModule'*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/utils/common.py#L24)
...
parent graph
- **comp_name** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - name for the new component
- **class_name** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - name for the submodule

Return type:

[tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[*GraphModule*, [dict](https://docs.python.org/3/builtins/stdtypes.html#dict)[[str](https://docs.python.org/3/builtins/stdtypes.html#str), [str](https://docs.python.org/3/builtins/stdtypes.html#str)]]
...
torch.fx.passes.utils.fuser_utils.fuse_as_graphmodule(*gm*, *nodes*, *module_name*, *partition_lookup_table=None*, ***, *always_return_tuple=False*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/utils/fuser_utils.py#L97)
...
- **module_name** ([*str*](https://docs.python.org/3/builtins/stdtypes.html#str)) - class name for the fused GraphModule
- **partition_lookup_table** (*Optional**[**Dict**[**Node**,**None**]**]*) - optional dict of nodes to speed up lookup
- **always_return_tuple** ([*bool*](https://docs.python.org/3/builtins/functions.html#bool)) - whether to always return a tuple, even if there is only one output
...
| --- | --- |
| [`topo_sort`](generated/torch.fx.passes.utils.fuser_utils.topo_sort.html#torch.fx.passes.utils.fuser_utils.topo_sort) | |
| [`validate_partition`](generated/torch.fx.passes.utils.fuser_utils.validate_partition.html#torch.fx.passes.utils.fuser_utils.validate_partition) | |

## torch.fx.passes.utils.source_matcher_utils

torch.fx.passes.utils.source_matcher_utils.get_source_partitions(*graph*, *wanted_sources*, *filter_fn=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/utils/source_matcher_utils.py#L55)

Parameters:

- **graph** (*Graph*) - The graph we want to partition
- **wanted_sources** ([*list*](https://docs.python.org/3/builtins/stdtypes.html#list)*[*[*Any*](https://docs.python.org/3/library/typing.html#typing.Any)*]*) - List of sources of nodes that were decomposed from this
...
[dict](https://docs.python.org/3/builtins/stdtypes.html#dict)[[*Any*](https://docs.python.org/3/library/typing.html#typing.Any), [list](https://docs.python.org/3/builtins/stdtypes.html#list)[*SourcePartition*]]
...
torch.fx.passes.annotate_getitem_nodes.annotate_getitem_nodes(*graph*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/annotate_getitem_nodes.py#L7)
...
torch.fx.passes.split_utils.move_non_tensor_nodes_on_boundary(*subgraphs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/split_utils.py#L346)
...
1. if it is in to_subgraph, return (continue DFS)
2. if it is in from_subgraph, collect the nodes to nodes_to_move, and continue DFS
3. otherwise, this means it cannot be moved
4. also check if node X's parent should be put into the queue. (The queue may
have duplicated nodes, just process the node once)

Parameters:

**subgraphs** ([*list*](https://docs.python.org/3/builtins/stdtypes.html#list)*[**Subgraph**]*) - List of subgraphs containing nodes to be processed

Warning

This API is experimental and is *NOT* backward-compatible.

torch.fx.passes.splitter_base.generate_inputs_for_submodules(*model*, *inputs*, *target_submodules*, *deepcopy=False*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/fx/passes/splitter_base.py#L440)
...
- **target_submodules** ([*Iterable*](https://docs.python.org/3/library/collections.abc.html#collections.abc.Iterable)*[*[*str*](https://docs.python.org/3/builtins/stdtypes.html#str)*]*) - submodules that we want to generate inputs for.
