...
constructor is [`dataset`](utils.html#module-torch.utils.data.dataset), which indicates a dataset object to load data
...
When using a `IterableDataset` with
...
`IterableDataset` documentations for how to
achieve this.
...
Alternatively, users may use the [`sampler`](utils.html#module-torch.utils.data.sampler) argument to specify a
...
Automatic batching can also be enabled via `batch_size` and
...
Neither [`sampler`](utils.html#module-torch.utils.data.sampler) nor `batch_sampler` is compatible with
...
`batch_size`, `drop_last`, `batch_sampler`, and
`collate_fn` (which has a default function).
...
batched samples instead of individual samples. `batch_size` and
...
to construct a `batch_sampler` from [`sampler`](utils.html#module-torch.utils.data.sampler). For map-style
datasets, the [`sampler`](utils.html#module-torch.utils.data.sampler) is either provided by user or constructed
based on the `shuffle` argument. For iterable-style datasets, the
[`sampler`](utils.html#module-torch.utils.data.sampler) is a dummy infinite one. See
...
the [`dataset`](utils.html#module-torch.utils.data.dataset) object.

When both `batch_size` and `batch_sampler` are `None` (default
value for `batch_sampler` is already `None`), automatic batching is
disabled. Each sample obtained from the [`dataset`](utils.html#module-torch.utils.data.dataset) is processed with the
...
`(image, class_index)`, the default `collate_fn` collates a list of
...
is created (e.g., when you call `enumerate(dataloader)`), `num_workers`
worker processes are created. At this point, the [`dataset`](utils.html#module-torch.utils.data.dataset),
`collate_fn`, and `worker_init_fn` are passed to each
...
[`sampler`](utils.html#module-torch.utils.data.sampler) and sends them to the workers. So any shuffle randomization is
...
[`dataset`](utils.html#module-torch.utils.data.dataset) object, naive multi-process loading will often result in
duplicated data. Using `torch.utils.data.get_worker_info()` and/or
...
Since workers rely on Python [`multiprocessing`](https://docs.python.org/3/library/multiprocessing.html#module-multiprocessing), worker launch behavior is
different on Windows compared to Unix.

- On Unix, The default [`multiprocessing`](https://docs.python.org/3/library/multiprocessing.html#module-multiprocessing) start method is `forkserver()`
for Python >= 3.14; `fork()` for Python < 3.14.
Using `fork()`, child workers typically can access the [`dataset`](utils.html#module-torch.utils.data.dataset) and
Python argument functions directly through the cloned address space. This can have a fast start up,
but can lead to problems with multi-threaded applications. On Unix platforms that support it, `forkserver()`
...
- On Windows and MacOS, `spawn()` is the default [`multiprocessing`](https://docs.python.org/3/library/multiprocessing.html#module-multiprocessing) start method.
Using `spawn()`, another interpreter is launched which runs your main script,
followed by the internal worker function that receives the [`dataset`](utils.html#module-torch.utils.data.dataset),
`collate_fn` and other arguments through [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle) serialization.
...
process is launched. You can place your dataset and `DataLoader`
instance creation logic here, as it doesn't need to be re-executed in workers.
- Make sure that any custom `collate_fn`, `worker_init_fn`
or [`dataset`](utils.html#module-torch.utils.data.dataset) code is declared as top level definitions, outside of the
...
or [`torch.initial_seed()`](generated/torch.initial_seed.html#torch.initial_seed), and use it to seed other libraries before data
loading.
...
For data loading, passing `pin_memory=True` to a
...
*class*torch.utils.data.DataLoader(*dataset*, *batch_size=1*, *shuffle=None*, *sampler=None*, *batch_sampler=None*, *num_workers=0*, *collate_fn=None*, *pin_memory=False*, *drop_last=False*, *timeout=0*, *worker_init_fn=None*, *multiprocessing_context=None*, *generator=None*, ***, *prefetch_factor=None*, *persistent_workers=False*, *pin_memory_device=''*, *in_order=True*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/dataloader.py#L149)
...
- **batch_sampler** (*Sampler**or**Iterable**,**optional*) - like [`sampler`](utils.html#module-torch.utils.data.sampler), but
...
If the `spawn` start method is used, `worker_init_fn`
cannot be an unpicklable object, e.g., a lambda function. See
[Multiprocessing best practices](notes/multiprocessing.html#multiprocessing-best-practices) on more details related
to multiprocessing in PyTorch.

Warning

`len(dataloader)` heuristic is based on the length of the sampler used.
When [`dataset`](utils.html#module-torch.utils.data.dataset) is an `IterableDataset`,
it instead returns an estimate based on `len(dataset) / batch_size`, with proper
rounding depending on `drop_last`, regardless of multi-process loading
configurations. This represents the best guess PyTorch can make because PyTorch
trusts user [`dataset`](utils.html#module-torch.utils.data.dataset) code in correctly handling multi-process
...
`IterableDataset` interacts with
Multi-process data loading.

Warning

See [Reproducibility](notes/randomness.html#reproducibility), and [My data loader workers return identical random numbers](notes/faq.html#dataloader-workers-random-seed), and
Randomness in multi-process data loading notes for random seed related questions.
...
*class*torch.utils.data.Dataset[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/dataset.py#L39)
...
`Sampler` implementations and the default options
...
*class*torch.utils.data.IterableDataset[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/dataset.py#L73)
...
*class*torch.utils.data.TensorDataset(**tensors*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/dataset.py#L189)
...
*class*torch.utils.data.StackDataset(**args*, ***kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/dataset.py#L212)
...
*class*torch.utils.data.ConcatDataset(*datasets*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/dataset.py#L299)
...
*class*torch.utils.data.ChainDataset(*datasets*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/dataset.py#L356)
...
*class*torch.utils.data.Subset(*dataset*, *indices*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/dataset.py#L386)
...
torch.utils.data._utils.collate.collate(*batch*, ***, *collate_fn_map=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/_utils/collate.py#L118)
...
torch.utils.data.default_collate(*batch*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/_utils/collate.py#L340)

Take in a batch of data and put the elements within the batch into a tensor with an additional outer dimension - batch size.

The exact output type can be a [`torch.Tensor`](tensors.html#torch.Tensor), a Sequence of [`torch.Tensor`](tensors.html#torch.Tensor), a
Collection of [`torch.Tensor`](tensors.html#torch.Tensor), or left unchanged, depending on the input type.
This is used as the default function for collation when
batch_size or batch_sampler is defined in `DataLoader`.

Here is the general input type (based on the type of the element within the batch) to output type mapping:

> - [`torch.Tensor`](tensors.html#torch.Tensor) -> [`torch.Tensor`](tensors.html#torch.Tensor) (with an added outer dimension batch size)
> - NumPy Arrays -> [`torch.Tensor`](tensors.html#torch.Tensor)
> - float -> [`torch.Tensor`](tensors.html#torch.Tensor)
> - int -> [`torch.Tensor`](tensors.html#torch.Tensor)
> - str -> str (unchanged)
...
torch.utils.data.default_convert(*data*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/_utils/collate.py#L23)

Convert each NumPy array element into a [`torch.Tensor`](tensors.html#torch.Tensor).

If the input is a Sequence, Collection, or Mapping, it tries to convert each element inside to a [`torch.Tensor`](tensors.html#torch.Tensor).
...
torch.utils.data.get_worker_info()[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/_utils/worker.py#L99)
...
- [`dataset`](utils.html#module-torch.utils.data.dataset): the copy of the dataset object in **this** process. Note
...
When used in a `worker_init_fn` passed over to
`DataLoader`, this method can be useful to
...
torch.utils.data.dataset.random_split(*dataset*, *lengths*, *generator=<torch._C.Generator object>*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/dataset.py#L449)
...
- **generator** ([*Generator*](generated/torch.Generator.html#torch.Generator)) - Generator used for the random permutation.

Return type:

[list](https://docs.python.org/3/builtins/stdtypes.html#list)[*Subset*[*_T*]]

*class*torch.utils.data.Sampler[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/sampler.py#L28)
...
*class*torch.utils.data.SequentialSampler(*data_source*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/sampler.py#L97)
...
*class*torch.utils.data.RandomSampler(*data_source*, *replacement=False*, *num_samples=None*, *generator=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/sampler.py#L116)
...
*class*torch.utils.data.SubsetRandomSampler(*indices*, *generator=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/sampler.py#L191)
...
*class*torch.utils.data.WeightedRandomSampler(*weights*, *num_samples*, *replacement=True*, *generator=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/sampler.py#L213)
...
*class*torch.utils.data.BatchSampler(*sampler*, *batch_size*, *drop_last*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/sampler.py#L286)
...
*class*torch.utils.data.distributed.DistributedSampler(*dataset*, *num_replicas=None*, *rank=None*, *shuffle=True*, *seed=0*, *drop_last=False*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/distributed.py#L17)

Sampler that restricts data loading to a subset of the dataset.

It is especially useful in conjunction with
[`torch.nn.parallel.DistributedDataParallel`](generated/torch.nn.parallel.DistributedDataParallel.html#torch.nn.parallel.DistributedDataParallel). In such a case, each
...
torch.utils.data.graph.traverse(*datapipe*, *only_datapipe=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/graph.py#L109)
...
[dict](https://docs.python.org/3/builtins/stdtypes.html#dict)[[int](https://docs.python.org/3/builtins/functions.html#int), [tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[*IterDataPipe* | *MapDataPipe*, [dict](https://docs.python.org/3/builtins/stdtypes.html#dict)[[int](https://docs.python.org/3/builtins/functions.html#int), [tuple](https://docs.python.org/3/builtins/stdtypes.html#tuple)[*IterDataPipe* | *MapDataPipe*, DataPipeGraph]]]]

torch.utils.data.graph.traverse_dps(*datapipe*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/graph.py#L91)
...
torch.utils.data.datapipes.utils.decoder.basichandlers(*extension*, *data*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/datapipes/utils/decoder.py#L32)
...
The data loaded into a corresponding python object
...
torch.utils.data.datapipes.utils.decoder.handle_extension(*extensions*, *f*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/datapipes/utils/decoder.py#L115)
...
torch.utils.data.datapipes.utils.common.validate_input_col(*fn*, *input_col*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/utils/data/datapipes/utils/common.py#L28)
...
| [`StreamWrapper`](generated/torch.utils.data.datapipes.utils.common.StreamWrapper.html#torch.utils.data.datapipes.utils.common.StreamWrapper) | StreamWrapper is introduced to wrap file handler generated by DataPipe operation like FileOpener. |
| --- | --- |