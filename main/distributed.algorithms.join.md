...
*class*torch.distributed.algorithms.Join(*joinables*, *enable=True*, *throw_on_early_termination=False*, ***kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/8d6343a7cd80821d54ba546bc6f799aad9ccb79c/torch/distributed/algorithms/join.py#L104)
...
erroring and to ensure algorithmic correctness. Refer to `JoinHook`
...
the `JoinHook` objects are the same. If there are multiple
`JoinHook` objects, then the `device` of the first is used.
...
order.
- **enable** ([*bool*](https://docs.python.org/3/builtins/functions.html#bool)) - a flag enabling uneven input detection; setting to
...
(default: `True`).
- **throw_on_early_termination** ([*bool*](https://docs.python.org/3/builtins/functions.html#bool)) - a flag controlling whether to throw an
...
*static*notify_join_context(*joinable*)[[source]](https://github.com/pytorch/pytorch/blob/8d6343a7cd80821d54ba546bc6f799aad9ccb79c/torch/distributed/algorithms/join.py#L298)
...
*class*torch.distributed.algorithms.Joinable[[source]](https://github.com/pytorch/pytorch/blob/8d6343a7cd80821d54ba546bc6f799aad9ccb79c/torch/distributed/algorithms/join.py#L44)

This defines an abstract base class for joinable classes.

A joinable class
(inheriting from `Joinable`) should implement `join_hook()`,
which returns a `JoinHook` instance, in addition to
`join_device()` and `join_process_group()` that return device and
process group information, respectively.

*abstract property*join_device*: [device](tensor_attributes.html#torch.device)*

Return the device from which to perform collective communications needed by the join context manager.

*abstract*join_hook(***kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/8d6343a7cd80821d54ba546bc6f799aad9ccb79c/torch/distributed/algorithms/join.py#L60)
...
*abstract property*join_process_group*: [Any](https://docs.python.org/3/library/typing.html#typing.Any)*

Returns the process group for the collective communications needed by the join context manager itself.

*class*torch.distributed.algorithms.JoinHook[[source]](https://github.com/pytorch/pytorch/blob/8d6343a7cd80821d54ba546bc6f799aad9ccb79c/torch/distributed/algorithms/join.py#L14)
...
main_hook()[[source]](https://github.com/pytorch/pytorch/blob/8d6343a7cd80821d54ba546bc6f799aad9ccb79c/torch/distributed/algorithms/join.py#L26)
...
post_hook(*is_last_joiner*)[[source]](https://github.com/pytorch/pytorch/blob/8d6343a7cd80821d54ba546bc6f799aad9ccb79c/torch/distributed/algorithms/join.py#L32)
...
**is_last_joiner** ([*bool*](https://docs.python.org/3/builtins/functions.html#bool)) - `True` if the rank is one of the last to
