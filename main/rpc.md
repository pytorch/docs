...
`rpc_sync()` (synchronous),
`rpc_async()` (asynchronous), and
`remote()` (asynchronous and returns a reference
...
caller. The `remote()` API is useful when the
...
`rpc_sync()` and
`rpc_async()` are no longer appropriate, as they
...
(the caller of `remote()`) or the owner of the
...
[`Optimizer()`](optim.html#torch.optim.Optimizer) (e.g., [`SGD()`](generated/torch.optim.SGD.html#torch.optim.SGD),
[`Adagrad()`](generated/torch.optim.Adagrad.html#torch.optim.Adagrad), etc.) and a list of parameter RRefs, creates an
[`Optimizer()`](optim.html#torch.optim.Optimizer) instance on each distinct RRef owner, and
...
torch.distributed.rpc.init_rpc(*name*, *backend=None*, *rank=-1*, *world_size=None*, *rpc_backend_options=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/__init__.py#L93)
...
torch.distributed.rpc.is_available()[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/__init__.py#L24)
...
torch.distributed.rpc.rpc_sync(*to*, *func*, *args=None*, *kwargs=None*, *timeout=-1.0*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/api.py#L771)
...
on both workers. Refer to [`init_process_group()`](distributed.html#torch.distributed.init_process_group)
...
torch.distributed.rpc.rpc_async(*to*, *func*, *args=None*, *kwargs=None*, *timeout=-1.0*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/api.py#L845)

Make a non-blocking RPC call to run function `func` on worker `to`. RPC
messages are sent and received in parallel to execution of Python code. This
method is thread-safe. This method will immediately return a
[`Future`](futures.html#torch.futures.Future) that can be awaited on.
...
Returns a [`Future`](futures.html#torch.futures.Future) object that can be waited
on. When completed, the return value of `func` on `args` and
`kwargs` can be retrieved from the [`Future`](futures.html#torch.futures.Future)
object.
...
[`Future`](futures.html#torch.futures.Future) completes.
...
torch.distributed.rpc.remote(*to*, *func*, *args=None*, *kwargs=None*, *timeout=-1.0*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/api.py#L554)
...
torch.distributed.rpc.get_worker_info(*worker_name=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/api.py#L427)
...
`WorkerInfo` instance for the given
`worker_name` or `WorkerInfo` of the
current worker if `worker_name` is `None`.

torch.distributed.rpc.shutdown(*graceful=True*, *timeout=0*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/api.py#L326)
...
For [`Future`](futures.html#torch.futures.Future) objects returned by
`rpc_async()`, `future.wait()` should not
...
torch.distributed.rpc.functions.async_execution(*fn*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/functions.py#L5)
...
callee extracts the [`Future`](futures.html#torch.futures.Future) returned by the wrapped
function and installs subsequent processing steps as a callback to that
[`Future`](futures.html#torch.futures.Future). The installed callback will read the value
from the [`Future`](futures.html#torch.futures.Future) when completed and send the
value back as the RPC response. That also means the returned
[`Future`](futures.html#torch.futures.Future) only exists on the callee side and is never
...
The returned [`Future`](futures.html#torch.futures.Future) object can come from
`rpc_async()`,
[`then()`](futures.html#torch.futures.Future.then), or [`Future`](futures.html#torch.futures.Future)
constructor. The example below shows directly using the
[`Future`](futures.html#torch.futures.Future) returned by
[`then()`](futures.html#torch.futures.Future.then).
...
torch.distributed.rpc.backend_registry.backend_registered(*backend_name*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/backend_registry.py#L51)
...
torch.distributed.rpc.backend_registry.register_backend(*backend_name*, *construct_rpc_backend_options_handler*, *init_backend_handler*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/backend_registry.py#L64)
...
the `BackendType` enum. Regardless of what backend
is used, the rest of the RPC API won't change. Each backend also defines its own
subclass of the `RpcBackendOptions` class, an
instance of which can also be passed to `init_rpc()`
to configure the backend's behavior.

*class*torch.distributed.rpc.BackendType(*value*, *names=None*, ***, *module=None*, *qualname=None*, *type=None*, *start=1*, *boundary=None*)
...
*class*torch.distributed.rpc.TensorPipeRpcBackendOptions(***, *num_worker_threads=16*, *rpc_timeout=60.0*, *init_method='env://'*, *device_maps=None*, *devices=None*, *_transports=None*, *_channels=None*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/options.py#L50)
...
set_device_map(*to*, *device_map*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/options.py#L111)
...
set_devices(*devices*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/rpc/options.py#L169)
...
`rpc_sync()`,
`rpc_async()` and
`remote()` calls. The reason being that there is
...
pickle, torch [`save()`](generated/torch.save.html#torch.save) / [`load()`](generated/torch.load.html#torch.load),
JIT `save()` / `load()`, etc.) will
lead to errors.
...
*class*torch.distributed.nn.api.remote_module.RemoteModule(**args*, ***kwargs*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/nn/api/remote_module.py#L605)
...
- **module_cls** ([*nn.Module*](generated/torch.nn.Module.html#torch.nn.Module)) -
...
get_module_rref()[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/nn/api/remote_module.py#L296)
...
remote_parameters(*recurse=True*)[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/nn/api/remote_module.py#L277)

Return a list of `RRef` pointing to the remote module's parameters.

This can typically be used in conjunction
with [`DistributedOptimizer`](distributed.optim.html#torch.distributed.optim.DistributedOptimizer).
...
*class*torch.distributed.autograd.context[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/autograd/__init__.py#L37)
...
torch.distributed.autograd.is_available()[[source]](https://github.com/pytorch/pytorch/blob/9b9978943e4030e97eeee36a9968db27a3b21163/torch/distributed/autograd/__init__.py#L12)
...
using torch.distributed.rpc APIs, and demonstrate how
