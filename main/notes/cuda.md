...
[`torch.cuda`](../cuda.html#module-torch.cuda) is used to set up and run CUDA operations. It keeps track of
...
[`torch.cuda.device`](../generated/torch.cuda.device.html#torch.cuda.device) context manager.
...
[`copy_()`](../generated/torch.Tensor.copy_.html#torch.Tensor.copy_) and other methods with copy-like functionality
such as [`to()`](../generated/torch.Tensor.to.html#torch.Tensor.to) and [`cuda()`](../generated/torch.Tensor.cuda.html#torch.Tensor.cuda).
...
The precision of matmuls can also be set more broadly (limited not just to CUDA) via [`set_float32_matmul_precision()`](../generated/torch.set_float32_matmul_precision.html#torch.set_float32_matmul_precision).
...
with FP16 accumulation than FP32 accumulation, see Full FP16 accumulation)
...
call [`torch.cuda.synchronize()`](../generated/torch.cuda.synchronize.html#torch.cuda.synchronize) before measuring, or use [`torch.cuda.Event`](../generated/torch.cuda.Event.html#torch.cuda.Event)
...
As an exception, several functions such as [`to()`](../generated/torch.Tensor.to.html#torch.Tensor.to) and
[`copy_()`](../generated/torch.Tensor.copy_.html#torch.Tensor.copy_) admit an explicit `non_blocking` argument,
...
[`synchronize()`](../generated/torch.cuda.synchronize.html#torch.cuda.synchronize) or [`wait_stream()`](../generated/torch.cuda.Stream_class.html#torch.cuda.Stream.wait_stream)) are
...
There are two new additions. The [`torch.cuda.Stream.wait_stream()`](../generated/torch.cuda.Stream_class.html#torch.cuda.Stream.wait_stream) call
ensures that the `normal_()` execution has finished before we start running
`sum(A)` on a side stream. The [`torch.Tensor.record_stream()`](../generated/torch.Tensor.record_stream.html#torch.Tensor.record_stream) (see for
...
documentation for [`torch.Tensor.record_stream()`](../generated/torch.Tensor.record_stream.html#torch.Tensor.record_stream) on more details on when
...
[`autograd.backward`](../generated/torch.autograd.backward.html#torch.autograd.backward),
[`autograd.grad`](../generated/torch.autograd.grad.html#torch.autograd.grad), or
[`tensor.backward`](../generated/torch.Tensor.backward.html#torch.Tensor.backward),
and optionally supplying CUDA tensor(s) as the initial gradient(s) (e.g.,
[`autograd.backward(..., grad_tensors=initial_grads)`](../generated/torch.autograd.backward.html#torch.autograd.backward),
[`autograd.grad(..., grad_outputs=initial_grads)`](../generated/torch.autograd.grad.html#torch.autograd.grad), or
[`tensor.backward(..., gradient=initial_grad)`](../generated/torch.Tensor.backward.html#torch.Tensor.backward)),
the acts of
...
[Understanding CUDA Memory Usage](../torch_cuda_memory.html#torch-cuda-memory) describes tools for capturing and visualizing traces of memory use.
...
can be split. The
`memory_stats()` and
`memory_summary()` methods are useful for tuning. This
...
[CUDA memory management API](../cuda.html#cuda-memory-management-api)
...
This can be used in python through the [`torch.cuda.memory.CUDAPluggableAllocator`](../generated/torch.cuda.memory.CUDAPluggableAllocator.html#torch.cuda.memory.CUDAPluggableAllocator).
...
`torch.cuda.MemPool`. This will let you use multiple CUDA system allocators in the same
...
The pool can then be used with the [`torch.cuda.use_mem_pool`](../cuda.html#torch.cuda.use_mem_pool) context manager to
...
The following `torch.cuda.MemPool.use_count()` and `torch.cuda.MemPool.snapshot()`
...
- `torch.cuda.MemPool` holds a reference to the pool. When you use the
[`torch.cuda.use_mem_pool`](../cuda.html#torch.cuda.use_mem_pool) context manager, it will also acquire another reference
...
running FFT methods (e.g., [`torch.fft.fft()`](../generated/torch.fft.fft.html#torch.fft.fft)) on CUDA tensors of same geometry
...
[`is_available()`](../generated/torch.cuda.is_available.html#torch.cuda.is_available). In the following, `args.device` results in a
[`torch.device`](../tensor_attributes.html#torch.device) object that can be used to move tensors to CPU or CUDA.
...
When assessing the availability of CUDA in a given environment ([`is_available()`](../generated/torch.cuda.is_available.html#torch.cuda.is_available)), PyTorch's default
...
[`is_available()`](../generated/torch.cuda.is_available.html#torch.cuda.is_available) will fail with a CUDA initialization error.

One can set `PYTORCH_NVML_BASED_CUDA_CHECK=1` in your environment before importing PyTorch modules that execute
[`is_available()`](../generated/torch.cuda.is_available.html#torch.cuda.is_available) (or before executing it directly) in order to direct
[`is_available()`](../generated/torch.cuda.is_available.html#torch.cuda.is_available) to attempt an NVML-based assessment ([nvmlDeviceGetCount_v2](https://docs.nvidia.com/deploy/nvml-api/group__nvmlDeviceQueries.html#group__nvmlDeviceQueries_1ga93623b195bff04bbe3490ca33c8a42d)). If the
NVML-based assessment is successful (i.e. NVML discovery/initialization does not fail),
[`is_available()`](../generated/torch.cuda.is_available.html#torch.cuda.is_available) calls will not poison subsequent forks.

If NVML discovery/initialization fails, [`is_available()`](../generated/torch.cuda.is_available.html#torch.cuda.is_available) will fallback to the standard CUDA Runtime
...
on, the best practice is to use a [`torch.cuda.device`](../generated/torch.cuda.device.html#torch.cuda.device) context manager.
...
(see [`torch.Tensor`](../tensors.html#torch.Tensor)).
...
fill it with either ones or zeros, [`ones_like()`](../generated/torch.ones_like.html#torch.ones_like) or
[`zeros_like()`](../generated/torch.zeros_like.html#torch.zeros_like) are provided as convenient helper functions (which
also preserve [`torch.device`](../tensor_attributes.html#torch.device) and [`torch.dtype`](../tensor_attributes.html#torch.dtype) of a Tensor).
...
[`to()`](../generated/torch.Tensor.to.html#torch.Tensor.to) or a [`cuda()`](../generated/torch.Tensor.cuda.html#torch.Tensor.cuda) call. This can be used
to overlap data transfers with computation.

You can make the [`DataLoader`](../data.html#torch.utils.data.DataLoader) return batches placed in
pinned memory by passing `pin_memory=True` to its constructor.

### Use nn.parallel.DistributedDataParallel instead of multiprocessing or nn.DataParallel

Most use cases involving batched inputs and multiple GPUs should default to
using [`DistributedDataParallel`](../generated/torch.nn.parallel.DistributedDataParallel.html#torch.nn.parallel.DistributedDataParallel) to utilize more
than one GPU.

There are significant caveats to using CUDA models with
[`multiprocessing`](../multiprocessing.html#module-torch.multiprocessing); unless care is taken to meet the data handling
requirements exactly, it is likely that your program will have incorrect or
undefined behavior.

It is recommended to use [`DistributedDataParallel`](../generated/torch.nn.parallel.DistributedDataParallel.html#torch.nn.parallel.DistributedDataParallel),
instead of [`DataParallel`](../generated/torch.nn.DataParallel.html#torch.nn.DataParallel) to do multi-GPU training, even if
there is only a single node.

The difference between [`DistributedDataParallel`](../generated/torch.nn.parallel.DistributedDataParallel.html#torch.nn.parallel.DistributedDataParallel) and
[`DataParallel`](../generated/torch.nn.DataParallel.html#torch.nn.DataParallel) is: [`DistributedDataParallel`](../generated/torch.nn.parallel.DistributedDataParallel.html#torch.nn.parallel.DistributedDataParallel)
uses multiprocessing where a process is created for each GPU, while
[`DataParallel`](../generated/torch.nn.DataParallel.html#torch.nn.DataParallel) uses multithreading. By using multiprocessing,
each GPU has its dedicated process, this avoids the performance overhead caused
by GIL of Python interpreter.

If you use [`DistributedDataParallel`](../generated/torch.nn.parallel.DistributedDataParallel.html#torch.nn.parallel.DistributedDataParallel), you could use
torch.distributed.launch utility to launch your program, see [Launch utility](../distributed.html#distributed-launch).
...
PyTorch exposes graphs via a raw [`torch.cuda.CUDAGraph`](../generated/torch.cuda.CUDAGraph.html#torch.cuda.CUDAGraph) class
and two convenience wrappers,
[`torch.cuda.graph`](../generated/torch.cuda.graph.html#torch.cuda.graph) and
[`torch.cuda.make_graphed_callables`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables).

[`torch.cuda.graph`](../generated/torch.cuda.graph.html#torch.cuda.graph) is a simple, versatile context manager that
...
Usage with torch.cuda.amp, and
Usage with multiple streams
for realistic and advanced patterns.

[`make_graphed_callables`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables) is more sophisticated.
[`make_graphed_callables`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables) accepts Python functions and
[`torch.nn.Module`](../generated/torch.nn.Module.html#torch.nn.Module)s. For each passed function or Module,
...
[`torch.cuda.graph`](../generated/torch.cuda.graph.html#torch.cuda.graph) context and all work in the forward and backward passes
of any callable you pass to [`torch.cuda.make_graphed_callables()`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables).
...
[`CUDAGraph.capture_begin`](../generated/torch.cuda.CUDAGraph.html#torch.cuda.CUDAGraph.capture_begin) and
[`CUDAGraph.capture_end`](../generated/torch.cuda.CUDAGraph.html#torch.cuda.CUDAGraph.capture_end) calls.
[`graph`](../generated/torch.cuda.graph.html#torch.cuda.graph) and
...
torch.cond(). See Data Dependent Control Flow.
...
part(s) eagerly and use [`torch.cuda.make_graphed_callables()`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables) to graph only
the capture-safe part(s).

By default, callables returned by [`make_graphed_callables()`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables)
are autograd-aware, and can be used in the training loop as direct replacements
for the functions or [`nn.Module`](../generated/torch.nn.Module.html#torch.nn.Module)s you passed.

[`make_graphed_callables()`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables) internally creates
[`CUDAGraph`](../generated/torch.cuda.CUDAGraph.html#torch.cuda.CUDAGraph) objects, runs warmup iterations, and maintains
static inputs and outputs as needed. Therefore (unlike with
[`torch.cuda.graph`](../generated/torch.cuda.graph.html#torch.cuda.graph)) you don't need to handle those manually.

In the following example, data-dependent dynamic control flow means the
network isn't capturable end-to-end, but
[`make_graphed_callables()`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables)
...
Call [`make_graphed_callables()`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables) on graphable network sections
*before* wrapping the network with DDP.

#### NCCL >= 2.9.6

NCCL versions 2.9.6 or later allow collectives in the graph.
Approaches that capture an entire backward pass
...
[`CUDAGraph`](../generated/torch.cuda.CUDAGraph.html#torch.cuda.CUDAGraph) object and all tensors created during capture
go out of scope.
...
To economize the memory stashed in private pools, [`torch.cuda.graph`](../generated/torch.cuda.graph.html#torch.cuda.graph)
and [`torch.cuda.make_graphed_callables()`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables) optionally allow different
...
[`torch.cuda.graph`](../generated/torch.cuda.graph.html#torch.cuda.graph)'s `pool` argument is a hint to use a particular private pool,
...
they share a pool, unless [`clone()`](../generated/torch.Tensor.clone.html#torch.Tensor.clone) is called on the outputs
beforehand.
...
With [`torch.cuda.make_graphed_callables()`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables), if you want to graph several
...
[`make_graphed_callables()`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables) will capture their graphs using a shared
private pool.
...
[`make_graphed_callables()`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables) is not allowed. Instead, you must call
[`make_graphed_callables()`](../generated/torch.cuda.make_graphed_callables.html#torch.cuda.make_graphed_callables) separately for each one.
