...
[`torch.utils.data.DataLoader`](../data.html#torch.utils.data.DataLoader)'s documentation for how to
...
[`Module`](../generated/torch.nn.Module.html#torch.nn.Module) with [`DataParallel`](../generated/torch.nn.DataParallel.html#torch.nn.DataParallel) or
[`data_parallel()`](../generated/torch.nn.functional.torch.nn.parallel.data_parallel.html#torch.nn.parallel.data_parallel). Input to each the `forward()` on
each device will only be part of the entire input. Because the unpack operation
[`torch.nn.utils.rnn.pad_packed_sequence()`](../generated/torch.nn.utils.rnn.pad_packed_sequence.html#torch.nn.utils.rnn.pad_packed_sequence) by default only pads up to the
...
[`pad_packed_sequence()`](../generated/torch.nn.utils.rnn.pad_packed_sequence.html#torch.nn.utils.rnn.pad_packed_sequence) to make sure that the
