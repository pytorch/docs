...
In this note we'll cover ways of extending [`torch.nn`](../nn.html#module-torch.nn),
[`torch.autograd`](../autograd.html#module-torch.autograd), [`torch`](../torch.html#module-torch), and writing custom C++ extensions.

## Adding new operators

PyTorch offers a large library of operators that work on Tensors (e.g. [`torch.add()`](../generated/torch.add.html#torch.add),
[`torch.sum()`](../generated/torch.sum.html#torch.sum), etc). However, you may wish to bring a new custom operation to PyTorch
...
## Extending [`torch.autograd`](../autograd.html#module-torch.autograd)

Adding operations to [`autograd`](../autograd.html#module-torch.autograd) requires implementing a new
[`Function`](../autograd.html#torch.autograd.Function) subclass for each operation. Recall that Functions
are what [`autograd`](../autograd.html#module-torch.autograd) uses to encode the operation history and compute
gradients.
...
you can wrap them in [`Function`](../autograd.html#torch.autograd.Function) to interface with the autograd
...
custom module. See the section below for more information on extending [`torch.nn`](../nn.html#module-torch.nn).
...
**Step 1:** After subclassing [`Function`](../autograd.html#torch.autograd.Function), you'll need to define 3 methods:

- [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) is the code that performs the operation. It can take
...
`Tensor` arguments that track history (i.e., with
...
- `setup_context()` (optional). One can either write a "combined" [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) that
accepts a `ctx` object or (as of PyTorch 2.0) a separate [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) that does
not accept `ctx` and a `setup_context()` method where the `ctx` modification happens.
The [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) should have the compute and `setup_context()` should
only be responsible for the `ctx` modification (and not have any compute).
In general the separate [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) and `setup_context()` is closer to how
...
- [`backward()`](../generated/torch.autograd.Function.backward.html#torch.autograd.Function.backward) (or `vjp()`) defines the gradient formula.
...
- [`save_for_backward()`](../generated/torch.autograd.function.FunctionCtx.save_for_backward.html#torch.autograd.function.FunctionCtx.save_for_backward) should be
...
[`torch.autograd.graph.saved_tensors_hooks`](../autograd.html#torch.autograd.graph.saved_tensors_hooks).

If tensors that are neither inputs nor outputs are saved for backward your
[`Function`](../autograd.html#torch.autograd.Function) may not support double backward (see step 3).
- [`mark_dirty()`](../generated/torch.autograd.function.FunctionCtx.mark_dirty.html#torch.autograd.function.FunctionCtx.mark_dirty) must be used to
mark any input that is modified inplace by the forward function.
- [`mark_non_differentiable()`](../generated/torch.autograd.function.FunctionCtx.mark_non_differentiable.html#torch.autograd.function.FunctionCtx.mark_non_differentiable) must
...
- [`set_materialize_grads()`](../generated/torch.autograd.function.FunctionCtx.set_materialize_grads.html#torch.autograd.function.FunctionCtx.set_materialize_grads) can be
...
[`input_grad_buffers`](../generated/torch.autograd.function.FunctionCtx.input_grad_buffers.html#torch.autograd.function.FunctionCtx.input_grad_buffers) for more details.
...
[`once_differentiable()`](../generated/torch.autograd.function.once_differentiable.html#torch.autograd.function.once_differentiable). With this decorator, attempts to
...
**Step 4:** It is recommended that you use [`torch.autograd.gradcheck()`](../autograd.html#module-torch.autograd.gradcheck)
...
If you need any "intermediate" Tensors computed in [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) to be saved,
...
[`Function`](../autograd.html#torch.autograd.Function)), higher order derivatives will work.
...
### Combined or separate [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) and `setup_context()`

There are two main ways to define [`Function`](../autograd.html#torch.autograd.Function). Either:

- define a [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) that combines the forward compute logic with `setup_context()`
- (as of PyTorch 2.0) define a separate [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) and `setup_context()`

We recommend the second option (separate [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) and `setup_context()`)
because that is closer to how PyTorch native operations are implemented and it composes
with [`torch.func`](../func.api.html#module-torch.func) transforms. However, we plan to support both approaches going forward;
combining [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) with `setup_context()`: leads to more flexibility since
you are able to save intermediates without returning them as output.

Please see the previous section for how to define [`Function`](../autograd.html#torch.autograd.Function) with separate
[`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) and `setup_context()`.

Here is an example of how to define a [`Function`](../autograd.html#torch.autograd.Function) with combined [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward) and
`setup_context()`:
...
You can implement the [`jvp()`](../generated/torch.autograd.Function.jvp.html#torch.autograd.Function.jvp) function.
...
The [`jvp()`](../generated/torch.autograd.Function.jvp.html#torch.autograd.Function.jvp) will be called just after the [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward)
...
- The [`jvp()`](../generated/torch.autograd.Function.jvp.html#torch.autograd.Function.jvp) function must match the view/inplace behavior of [`forward()`](../generated/torch.autograd.Function.forward.html#torch.autograd.Function.forward).
...
## Extending [`torch.nn`](../nn.html#module-torch.nn)

[`nn`](../nn.html#module-torch.nn) exports two kinds of interfaces - modules and their functional
...
### Adding a [`Module`](../generated/torch.nn.Module.html#torch.nn.Module)

Since [`nn`](../nn.html#module-torch.nn) heavily utilizes [`autograd`](../autograd.html#module-torch.autograd), adding a new
[`Module`](../generated/torch.nn.Module.html#torch.nn.Module) requires implementing a [`Function`](../autograd.html#torch.autograd.Function)
...
- [`forward()`](../generated/torch.nn.Module.html#torch.nn.Module.forward) - instantiates a [`Function`](../autograd.html#torch.autograd.Function) and
...
## Extending [`torch`](../torch.html#module-torch) Python API
...
to pass these types to functions like [`torch.add()`](../generated/torch.add.html#torch.add) in the top-level
[`torch`](../torch.html#module-torch) namespace that accept `Tensor` operands?
...
custom class is passed to a function in the [`torch`](../torch.html#module-torch) namespace. This makes
it possible to define custom implementations for any of the functions in the
[`torch`](../torch.html#module-torch) namespace which your `__torch_function__` implementation can call,
...
### Extending [`torch`](../torch.html#module-torch) with a `Tensor`-like type
...
If we try to use this object with the [`torch`](../torch.html#module-torch) API, we will run
into issues:
...
[`torch.add()`](../generated/torch.add.html#torch.add):
...
keyword arguments like [`torch.add()`](../generated/torch.add.html#torch.add) does:
...
function being overridden in the [`torch`](../torch.html#module-torch) API. For some applications ignoring
...
Functions in the [`torch`](../torch.html#module-torch) API that do not have explicit overrides will
...
explicitly implement the full [`torch`](../torch.html#module-torch) API or the entire subset of the API
that you care about for your use case. This may be a tall order as the full
[`torch`](../torch.html#module-torch) API is quite extensive.
...
Then [`torch.mul()`](../generated/torch.mul.html#torch.mul) will work correctly, although the return type will always
...
through operations in the [`torch`](../torch.html#module-torch) API.
...
### Extending [`torch`](../torch.html#module-torch) with a `Tensor` wrapper type
...
`Tensor` that is propagated through [`torch`](../torch.html#module-torch) operations. Since this
is a generic sort of wrapping for the full [`torch`](../torch.html#module-torch) API, we do not need to
...
[`torch`](../torch.html#module-torch) API but it is good enough to capture most common operations:
...
## Extending [`torch`](../torch.html#module-torch) native API
...
`Tensor` matches a `Tensor` parameter. For example, the generated
...
[`torch.library.custom_op()`](../library.html#torch.library.custom_op), should therefore use `Scalar` for Python
...
## Extending all [`torch`](../torch.html#module-torch) API with Modes
