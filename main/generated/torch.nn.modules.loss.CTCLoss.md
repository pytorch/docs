...
*class*torch.nn.modules.loss.CTCLoss(*blank=0*, *reduction='mean'*, *zero_infinity=False*)[[source]](https://github.com/pytorch/pytorch/blob/f744a6b99cda942b3dd232f56c0ebf413660c13f/torch/nn/modules/loss.py#L2151)
...
[`torch.nn.functional.log_softmax()`](torch.nn.functional.log_softmax.html#torch.nn.functional.log_softmax)).
...
where N=batch sizeN = \text{batch size}N=batch size. It represents the lengths of the
...
where N=batch sizeN = \text{batch size}N=batch size. It represents lengths of the targets.
...
In order to use CuDNN, the following must be satisfied: the `targets` must be
in concatenated format, all `input_lengths` must be T. blank=0blank=0blank=0,
`target_lengths` ≤256\leq 256≤256, the integer arguments must be of
dtype `torch.int32`, and the `log_probs` itself must be of
dtype `torch.float32`.
...
forward(*log_probs*, *targets*, *input_lengths*, *target_lengths*)[[source]](https://github.com/pytorch/pytorch/blob/f744a6b99cda942b3dd232f56c0ebf413660c13f/torch/nn/modules/loss.py#L2313)
