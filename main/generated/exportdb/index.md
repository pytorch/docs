...
ExportedProgram:
 class GraphModule(torch.nn.Module):
 def forward(self, x: "f32[s6, 2]", y: "f32[2]"):
 # No stacktrace found for following nodes
 sym_size_int_1: "Sym(s6)" = torch.ops.aten.sym_size.int(x, 0)

 gt: "Sym(s6 > 2)" = sym_size_int_1 > 2; sym_size_int_1 = None
 true_graph_0 = self.true_graph_0
 false_graph_0 = self.false_graph_0
 cond = torch.ops.higher_order.cond(gt, true_graph_0, false_graph_0, (x, y)); gt = true_graph_0 = false_graph_0 = x = y = None
 getitem: "f32[s6, 2]" = cond[0]; cond = None
 return (getitem,)

 class true_graph_0(torch.nn.Module):
 def forward(self, x: "f32[s6, 2]", y: "f32[2]"):
 add: "f32[s6, 2]" = torch.ops.aten.add.Tensor(x, y); x = y = None
 return (add,)

 class false_graph_0(torch.nn.Module):
 def forward(self, x: "f32[s6, 2]", y: "f32[2]"):
 sub: "f32[s6, 2]" = torch.ops.aten.sub.Tensor(x, y); x = y = None
 return (sub,)
...
Tags: [torch.dynamic-shape](torch.dynamic-shape.html), [python.assert](python.assert.html), [python.data-structure](python.data-structure.html)
...
ExportedProgram:
 class GraphModule(torch.nn.Module):
 def forward(self, c_dict_type_l__self______dict_____forward_____defaults___0: "f32[2, 3]", x: "f32[2, 3]"):
 add: "f32[2, 3]" = torch.ops.aten.add.Tensor(x, c_dict_type_l__self______dict_____forward_____defaults___0); x = c_dict_type_l__self______dict_____forward_____defaults___0 = None
 return (add,)
...
ExportedProgram:
 class GraphModule(torch.nn.Module):
 def forward(self, x: "f32[3, s11]"):
 # No stacktrace found for following nodes
 sym_size_int_1: "Sym(s11)" = torch.ops.aten.sym_size.int(x, 1); x = None

 add: "Sym(s11 + 1)" = sym_size_int_1 + 1; sym_size_int_1 = None
 return (add,)
...
Range constraints: {s11: VR[0, int_oo]}
```
...
```
RuntimeError: Constraints violated (dim0_x)! For more information, run with TORCH_LOGS="+dynamic".
```