...
torch.vander(*x*, *N=None*, *increasing=False*) → [Tensor](../tensors.html#torch.Tensor)

Generates a Vandermonde matrix.

The columns of the output matrix are elementwise powers of the input vector x(N−1),x(N−2),...,x0x^{(N-1)}, x^{(N-2)}, ..., x^0x(N−1),x(N−2),...,x0.
...
Vandermonde matrix. If increasing is False, the first column is x(N−1)x^{(N-1)}x(N−1),
