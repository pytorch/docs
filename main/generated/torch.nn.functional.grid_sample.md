...
torch.nn.functional.grid_sample(*input*, *grid*, *mode='bilinear'*, *padding_mode='zeros'*, *align_corners=None*)[[source]](https://github.com/pytorch/pytorch/blob/2a8ba15825312e681c7dc6b12b79dec216aecd30/torch/nn/functional.py#L5497)
...
(N,C,Hin,Win)(N, C, H_\text{in}, W_\text{in})(N,C,Hin​,Win​) and `grid` with shape
(N,Hout,Wout,2)(N, H_\text{out}, W_\text{out}, 2)(N,Hout​,Wout​,2), the output will have shape
...
This function is often used in conjunction with [`affine_grid()`](torch.nn.functional.affine_grid.html#torch.nn.functional.affine_grid)
...
- **input** ([*Tensor*](../tensors.html#torch.Tensor)) - input of shape (N,C,Hin,Win)(N, C, H_\text{in}, W_\text{in})(N,C,Hin​,Win​) (4-D case)
...
[`interpolate()`](torch.nn.functional.interpolate.html#torch.nn.functional.interpolate), and so whichever option is used here
...
`mode='bicubic'` is implemented using the [cubic convolution algorithm](https://en.wikipedia.org/wiki/Bicubic_interpolation) with α=−0.75\alpha=-0.75α=−0.75.
The constant α\alphaα might be different from packages to packages.
...
Clamp the results with [`torch.clamp()`](torch.clamp.html#torch.clamp) to ensure they are within the valid range.