...
the size of the datatype passed to the [`dtype`](../tensor_attributes.html#torch.dtype) keyword argument. If no [`dtype`](../tensor_attributes.html#torch.dtype)
is passed then it is inferred from the buffer's format, and an error is raised if the
format cannot be mapped to a PyTorch datatype, in which case [`dtype`](../tensor_attributes.html#torch.dtype) must be passed
