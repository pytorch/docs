...
corresponding dimension. The [`range`](torch.range.html#torch.range) argument can be provided to manually
...
See also [`torch.histogram()`](torch.histogram.html#torch.histogram), which specifically computes 1D histograms.
While `torch.histogramdd()` infers the dimensionality of its bins and
binned values from the shape of `input`, [`torch.histogram()`](torch.histogram.html#torch.histogram)
accepts and flattens `input` of any shape.
