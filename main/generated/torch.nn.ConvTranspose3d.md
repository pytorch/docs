...
*class*torch.nn.ConvTranspose3d(*in_channels*, *out_channels*, *kernel_size*, *stride=1*, *padding=0*, *output_padding=0*, *groups=1*, *bias=True*, *dilation=1*, *padding_mode='zeros'*, *device=None*, *dtype=None*)[[source]](https://github.com/pytorch/pytorch/blob/7e49a76253edc1ab706e08750fcdacd6cfc5e114/torch/nn/modules/conv.py#L1210)
...
when a [`Conv3d`](torch.nn.Conv3d.html#torch.nn.Conv3d) and a `ConvTranspose3d`
are initialized with same parameters, they are inverses of each other in
regard to the input and output shapes. However, when `stride > 1`,
[`Conv3d`](torch.nn.Conv3d.html#torch.nn.Conv3d) maps multiple input shapes to the same output
...
k=groupsCout∗∏i=02kernel_size[i]k = \frac{groups}{C_\text{out} * \prod_{i=0}^{2}\text{kernel\_size}[i]}k=Cout​∗∏i=02​kernel_size[i]groups​
