...
torch.linalg.lu(*A*, ***, *pivot=True*, *out=None*)[[source]](https://github.com/pytorch/pytorch/blob/e7003ce301964b7a4ef5d2d4777331489745a93c/torch/linalg/__init__.py#L2641)
...
A=PLUP∈Km×m,L∈Km×k,U∈Kk×nA = PLU\mathrlap{\qquad P \in \mathbb{K}^{m \times m}, L \in \mathbb{K}^{m \times k}, U \in \mathbb{K}^{k \times n}}A=PLUP∈Km×m,L∈Km×k,U∈Kk×n

where k = min(m,n), PPP is a [permutation matrix](https://en.wikipedia.org/wiki/Permutation_matrix), LLL is lower triangular with ones on the diagonal
and UUU is upper triangular.

If `pivot`= False and `A` is on GPU, then the **LU decomposition without pivoting** is computed

A=LUL∈Km×k,U∈Kk×nA = LU\mathrlap{\qquad L \in \mathbb{K}^{m \times k}, U \in \mathbb{K}^{k \times n}}A=LUL∈Km×k,U∈Kk×n
...
[`torch.linalg.solve()`](torch.linalg.solve.html#torch.linalg.solve) solves a system of linear equations using the LU decomposition
