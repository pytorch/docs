...
- A nested graph break results in O(N)\mathcal O(N)O(N) duplicate graph break behavior
...
Note that the runtime of handling this graph break is O(NK)\mathcal O(NK)O(NK), where NNN is the nesting depth,
and KKK is the number of instructions from the top-level function to the graph break.
We end up tracing O(N2)\mathcal O(N^2)O(N2) frames, and we trace the same graph break O(N)\mathcal O(N)O(N) times.