...
By default, PyTorch stores [`torch.Tensor`](tensors.html#torch.Tensor) elements contiguously in
...
PyTorch currently supports COO, CSR,
CSC, BSR, and BSC.

We also have a prototype implementation to support semi-structured sparsity.
...
See our operator documentation for a list.
...
Mdense=r×c×eMsparse=Mspecified+Mmetadata=r×c2×e+r×c2×2=rce2+rc=rce(12+1e)M_{dense} = r \times c \times e \\
...
torch.sparse.semi_structured.to_sparse_semi_structured(*original_tensor*, *transposed=False*, *alg_id=0*)[[source]](https://github.com/pytorch/pytorch/blob/e3b3670d208b9e770a7ca36a3fed1ea0f052f799/torch/sparse/semi_structured.py#L309)
...
| [`SparseSemiStructuredTensorCUSPARSELT`](generated/torch.sparse.semi_structured.SparseSemiStructuredTensorCUSPARSELT.html#torch.sparse.semi_structured.SparseSemiStructuredTensorCUSPARSELT) | The cuSPARSELt backend expects the specified elements and the metadata to be stored in a single tensor: packed = [ specified elements of original tensor \| metadata ] For an original tensor of size (m, k) we expect the first m * k // 2 elements to be the kept elements The rest of the tensor is metadata. |
| --- | --- |
| [`SparseSemiStructuredTensorCUTLASS`](generated/torch.sparse.semi_structured.SparseSemiStructuredTensorCUTLASS.html#torch.sparse.semi_structured.SparseSemiStructuredTensorCUTLASS) | This class implements semi-structured sparsity for the CUTLASS backend. |
...
[`torch.sparse_coo_tensor()`](generated/torch.sparse_coo_tensor.html#torch.sparse_coo_tensor).
...
globally using [`torch.sparse.check_sparse_tensor_invariants`](generated/torch.sparse.check_sparse_tensor_invariants.html#torch.sparse.check_sparse_tensor_invariants)
...
In general, the output of [`torch.Tensor.coalesce()`](generated/torch.Tensor.coalesce.html#torch.Tensor.coalesce) method is a
...
- [`torch.Tensor.is_coalesced()`](generated/torch.Tensor.is_coalesced.html#torch.Tensor.is_coalesced) returns `True`.
...
entries (e.g., [`torch.Tensor.add()`](generated/torch.Tensor.add.html#torch.Tensor.add)), you should occasionally
...
As mentioned above, a sparse COO tensor is a [`torch.Tensor`](tensors.html#torch.Tensor)
instance and to distinguish it from the `Tensor` instances that use
some other layout, one can use [`torch.Tensor.is_sparse`](generated/torch.Tensor.is_sparse.html#torch.Tensor.is_sparse) or
...
methods [`torch.Tensor.sparse_dim()`](generated/torch.Tensor.sparse_dim.html#torch.Tensor.sparse_dim) and
[`torch.Tensor.dense_dim()`](generated/torch.Tensor.dense_dim.html#torch.Tensor.dense_dim), respectively. For instance:
...
acquired using methods [`torch.Tensor.indices()`](generated/torch.Tensor.indices.html#torch.Tensor.indices) and
[`torch.Tensor.values()`](generated/torch.Tensor.values.html#torch.Tensor.values).
...
the [`torch.Tensor.coalesce()`](generated/torch.Tensor.coalesce.html#torch.Tensor.coalesce) method:
...
instance, [`torch.sparse.softmax()`](generated/torch.sparse.softmax.html#torch.sparse.softmax) computes the softmax with the
...
while the shape of the sparse CSR tensor is `(*batchsize, nrows, ncols, *densesize)` where `len(batchsize) == B` and
`len(densesize) == K`.
...
The number of sparse and dense dimensions can be acquired using
[`torch.Tensor.sparse_dim()`](generated/torch.Tensor.sparse_dim.html#torch.Tensor.sparse_dim) and [`torch.Tensor.dense_dim()`](generated/torch.Tensor.dense_dim.html#torch.Tensor.dense_dim)
...
With the same example data of the note in sparse COO format introduction, the memory consumption of a 10 000
...
[`torch.sparse_csr_tensor()`](generated/torch.sparse_csr_tensor.html#torch.sparse_csr_tensor) function. The user must supply the row
...
[`torch.Tensor.to_sparse_csr()`](generated/torch.Tensor.to_sparse_csr.html#torch.Tensor.to_sparse_csr) method. Any zeros in the (strided)
...
Similarly to sparse CSR tensors, a sparse CSC
...
[`torch.sparse_csc_tensor()`](generated/torch.sparse_csc_tensor.html#torch.sparse_csc_tensor) function. The user must supply the row
...
any two-dimensional tensor using [`torch.Tensor.to_sparse_csc()`](generated/torch.Tensor.to_sparse_csc.html#torch.Tensor.to_sparse_csc)
...
- The `crow_indices` tensor consists of compressed row
indices. This is a (B + 1)-D tensor of shape `(*batchsize, nrowblocks + 1)`. The last element is the number of specified blocks,
...
[`torch.sparse_bsr_tensor()`](generated/torch.sparse_bsr_tensor.html#torch.sparse_bsr_tensor) function. The user must supply the row
...
any two-dimensional tensor using [`torch.Tensor.to_sparse_bsr()`](generated/torch.Tensor.to_sparse_bsr.html#torch.Tensor.to_sparse_bsr)
...
- The `ccol_indices` tensor consists of compressed column
indices. This is a (B + 1)-D tensor of shape `(*batchsize, ncolblocks + 1)`. The last element is the number of specified blocks,
`nse`. This tensor encodes the index in `values` and
...
[`torch.sparse_bsc_tensor()`](generated/torch.sparse_bsc_tensor.html#torch.sparse_bsc_tensor) function. The user must supply the row
...
[`torch.sparse_compressed_tensor()`](generated/torch.sparse_compressed_tensor.html#torch.sparse_compressed_tensor) function that have the same
interface as the above discussed constructor functions
[`torch.sparse_csr_tensor()`](generated/torch.sparse_csr_tensor.html#torch.sparse_csr_tensor), [`torch.sparse_csc_tensor()`](generated/torch.sparse_csc_tensor.html#torch.sparse_csc_tensor),
[`torch.sparse_bsr_tensor()`](generated/torch.sparse_bsr_tensor.html#torch.sparse_bsr_tensor), and [`torch.sparse_bsc_tensor()`](generated/torch.sparse_bsc_tensor.html#torch.sparse_bsc_tensor),
...
layout parameter to the [`torch.sparse_compressed_tensor()`](generated/torch.sparse_compressed_tensor.html#torch.sparse_compressed_tensor)
function:
...
| --- | --- | --- |
| [`torch.mv()`](generated/torch.mv.html#torch.mv) | no | `M[sparse_coo] @ V[strided] -> V[strided]` |
| [`torch.mv()`](generated/torch.mv.html#torch.mv) | no | `M[sparse_csr] @ V[strided] -> V[strided]` |
| [`torch.matmul()`](generated/torch.matmul.html#torch.matmul) | no | `M[sparse_coo] @ M[strided] -> M[strided]` |
| [`torch.matmul()`](generated/torch.matmul.html#torch.matmul) | no | `M[sparse_csr] @ M[strided] -> M[strided]` |
| [`torch.matmul()`](generated/torch.matmul.html#torch.matmul) | no | `M[SparseSemiStructured] @ M[strided] -> M[strided]` |
| [`torch.matmul()`](generated/torch.matmul.html#torch.matmul) | no | `M[strided] @ M[SparseSemiStructured] -> M[strided]` |
| [`torch.mm()`](generated/torch.mm.html#torch.mm) | no | `M[sparse_coo] @ M[strided] -> M[strided]` |
| [`torch.mm()`](generated/torch.mm.html#torch.mm) | no | `M[SparseSemiStructured] @ M[strided] -> M[strided]` |
| [`torch.mm()`](generated/torch.mm.html#torch.mm) | no | `M[strided] @ M[SparseSemiStructured] -> M[strided]` |
| [`torch.sparse.mm()`](generated/torch.sparse.mm.html#torch.sparse.mm) | yes | `M[sparse_coo] @ M[strided] -> M[strided]` |
| [`torch.smm()`](generated/torch.smm.html#torch.smm) | no | `M[sparse_coo] @ M[strided] -> M[sparse_coo]` |
| [`torch.hspmm()`](generated/torch.hspmm.html#torch.hspmm) | no | `M[sparse_coo] @ M[strided] -> M[hybrid sparse_coo]` |
| [`torch.bmm()`](generated/torch.bmm.html#torch.bmm) | no | `T[sparse_coo] @ T[strided] -> T[strided]` |
| [`torch.addmm()`](generated/torch.addmm.html#torch.addmm) | no | `f * M[strided] + f * (M[sparse_coo] @ M[strided]) -> M[strided]` |
| [`torch.addmm()`](generated/torch.addmm.html#torch.addmm) | no | `f * M[strided] + f * (M[SparseSemiStructured] @ M[strided]) -> M[strided]` |
| [`torch.addmm()`](generated/torch.addmm.html#torch.addmm) | no | `f * M[strided] + f * (M[strided] @ M[SparseSemiStructured]) -> M[strided]` |
| [`torch.sparse.addmm()`](generated/torch.sparse.addmm.html#torch.sparse.addmm) | yes | `f * M[strided] + f * (M[sparse_coo] @ M[strided]) -> M[strided]` |
| [`torch.sparse.spsolve()`](generated/torch.sparse.spsolve.html#torch.sparse.spsolve) | no | `SOLVE(M[sparse_csr], V[strided]) -> V[strided]` |
| [`torch.sspaddmm()`](generated/torch.sspaddmm.html#torch.sspaddmm) | no | `f * M[sparse_coo] + f * (M[sparse_coo] @ M[strided]) -> M[sparse_coo]` |
| [`torch.lobpcg()`](generated/torch.lobpcg.html#torch.lobpcg) | no | `GENEIG(M[sparse_coo]) -> M[strided], M[strided]` |
| [`torch.pca_lowrank()`](generated/torch.pca_lowrank.html#torch.pca_lowrank) | yes | `PCA(M[sparse_coo]) -> M[strided], M[strided], M[strided]` |
| [`torch.svd_lowrank()`](generated/torch.svd_lowrank.html#torch.svd_lowrank) | yes | `SVD(M[sparse_coo]) -> M[strided], M[strided], M[strided]` |
...
except [`torch.smm()`](generated/torch.smm.html#torch.smm), support backward with respect to strided
matrix arguments.
...
| --- | --- |
| [`Tensor.is_sparse_csr`](generated/torch.Tensor.is_sparse_csr.html#torch.Tensor.is_sparse_csr) | Is `True` if the Tensor uses sparse CSR storage layout, `False` otherwise. |
| [`Tensor.dense_dim`](generated/torch.Tensor.dense_dim.html#torch.Tensor.dense_dim) | Return the number of dense dimensions in a sparse tensor `self`. |
| [`Tensor.sparse_dim`](generated/torch.Tensor.sparse_dim.html#torch.Tensor.sparse_dim) | Return the number of sparse dimensions in a sparse tensor `self`. |
| [`Tensor.sparse_mask`](generated/torch.Tensor.sparse_mask.html#torch.Tensor.sparse_mask) | Returns a new sparse tensor with values from a strided tensor `self` filtered by the indices of the sparse tensor `mask`. |
| [`Tensor.to_sparse`](generated/torch.Tensor.to_sparse.html#torch.Tensor.to_sparse) | Returns a sparse copy of the tensor. |
| [`Tensor.to_sparse_coo`](generated/torch.Tensor.to_sparse_coo.html#torch.Tensor.to_sparse_coo) | Convert a tensor to coordinate format. |
...
| [`Tensor.values`](generated/torch.Tensor.values.html#torch.Tensor.values) | Return the values tensor of a sparse COO tensor. |

The following Tensor methods are specific to sparse COO tensors:

| [`Tensor.coalesce`](generated/torch.Tensor.coalesce.html#torch.Tensor.coalesce) | Returns a coalesced copy of `self` if `self` is an uncoalesced tensor. |
| --- | --- |
| [`Tensor.sparse_resize_`](generated/torch.Tensor.sparse_resize_.html#torch.Tensor.sparse_resize_) | Resizes `self` sparse tensor to the desired size and the number of sparse and dense dimensions. |
| [`Tensor.sparse_resize_and_clear_`](generated/torch.Tensor.sparse_resize_and_clear_.html#torch.Tensor.sparse_resize_and_clear_) | Removes all specified elements from a sparse tensor `self` and resizes `self` to the desired size and the number of sparse and dense dimensions. |
| [`Tensor.is_coalesced`](generated/torch.Tensor.is_coalesced.html#torch.Tensor.is_coalesced) | Returns `True` if `self` is a sparse COO tensor that is coalesced, `False` otherwise. |
| [`Tensor.indices`](generated/torch.Tensor.indices.html#torch.Tensor.indices) | Return the indices tensor of a sparse COO tensor. |

The following methods are specific to sparse CSR tensors and sparse BSR tensors:

| [`Tensor.crow_indices`](generated/torch.Tensor.crow_indices.html#torch.Tensor.crow_indices) | Returns the tensor containing the compressed row indices of the `self` tensor when `self` is a sparse CSR tensor of layout `sparse_csr`. |
| --- | --- |
| [`Tensor.col_indices`](generated/torch.Tensor.col_indices.html#torch.Tensor.col_indices) | Returns the tensor containing the column indices of the `self` tensor when `self` is a sparse CSR tensor of layout `sparse_csr`. |

The following methods are specific to sparse CSC tensors and sparse BSC tensors:

| [`Tensor.row_indices`](generated/torch.Tensor.row_indices.html#torch.Tensor.row_indices) | |
| --- | --- |
| [`Tensor.ccol_indices`](generated/torch.Tensor.ccol_indices.html#torch.Tensor.ccol_indices) | |

The following Tensor methods support sparse COO tensors:

[`add()`](generated/torch.Tensor.add.html#torch.Tensor.add)
[`add_()`](generated/torch.Tensor.add_.html#torch.Tensor.add_)
[`addmm()`](generated/torch.Tensor.addmm.html#torch.Tensor.addmm)
[`addmm_()`](generated/torch.Tensor.addmm_.html#torch.Tensor.addmm_)
[`any()`](generated/torch.Tensor.any.html#torch.Tensor.any)
[`asin()`](generated/torch.Tensor.asin.html#torch.Tensor.asin)
[`asin_()`](generated/torch.Tensor.asin_.html#torch.Tensor.asin_)
[`arcsin()`](generated/torch.Tensor.arcsin.html#torch.Tensor.arcsin)
[`arcsin_()`](generated/torch.Tensor.arcsin_.html#torch.Tensor.arcsin_)
[`bmm()`](generated/torch.Tensor.bmm.html#torch.Tensor.bmm)
[`clone()`](generated/torch.Tensor.clone.html#torch.Tensor.clone)
[`deg2rad()`](generated/torch.Tensor.deg2rad.html#torch.Tensor.deg2rad)
`deg2rad_()`
[`detach()`](generated/torch.Tensor.detach.html#torch.Tensor.detach)
[`detach_()`](generated/torch.Tensor.detach_.html#torch.Tensor.detach_)
[`dim()`](generated/torch.Tensor.dim.html#torch.Tensor.dim)
[`div()`](generated/torch.Tensor.div.html#torch.Tensor.div)
[`div_()`](generated/torch.Tensor.div_.html#torch.Tensor.div_)
[`floor_divide()`](generated/torch.Tensor.floor_divide.html#torch.Tensor.floor_divide)
[`floor_divide_()`](generated/torch.Tensor.floor_divide_.html#torch.Tensor.floor_divide_)
[`get_device()`](generated/torch.Tensor.get_device.html#torch.Tensor.get_device)
[`index_select()`](generated/torch.Tensor.index_select.html#torch.Tensor.index_select)
[`isnan()`](generated/torch.Tensor.isnan.html#torch.Tensor.isnan)
[`log1p()`](generated/torch.Tensor.log1p.html#torch.Tensor.log1p)
[`log1p_()`](generated/torch.Tensor.log1p_.html#torch.Tensor.log1p_)
[`mm()`](generated/torch.Tensor.mm.html#torch.Tensor.mm)
[`mul()`](generated/torch.Tensor.mul.html#torch.Tensor.mul)
[`mul_()`](generated/torch.Tensor.mul_.html#torch.Tensor.mul_)
[`mv()`](generated/torch.Tensor.mv.html#torch.Tensor.mv)
[`narrow_copy()`](generated/torch.Tensor.narrow_copy.html#torch.Tensor.narrow_copy)
[`neg()`](generated/torch.Tensor.neg.html#torch.Tensor.neg)
[`neg_()`](generated/torch.Tensor.neg_.html#torch.Tensor.neg_)
[`negative()`](generated/torch.Tensor.negative.html#torch.Tensor.negative)
[`negative_()`](generated/torch.Tensor.negative_.html#torch.Tensor.negative_)
[`numel()`](generated/torch.Tensor.numel.html#torch.Tensor.numel)
[`rad2deg()`](generated/torch.Tensor.rad2deg.html#torch.Tensor.rad2deg)
`rad2deg_()`
[`resize_as_()`](generated/torch.Tensor.resize_as_.html#torch.Tensor.resize_as_)
[`size()`](generated/torch.Tensor.size.html#torch.Tensor.size)
[`pow()`](generated/torch.Tensor.pow.html#torch.Tensor.pow)
[`sqrt()`](generated/torch.Tensor.sqrt.html#torch.Tensor.sqrt)
[`square()`](generated/torch.Tensor.square.html#torch.Tensor.square)
[`smm()`](generated/torch.Tensor.smm.html#torch.Tensor.smm)
[`sspaddmm()`](generated/torch.Tensor.sspaddmm.html#torch.Tensor.sspaddmm)
[`sub()`](generated/torch.Tensor.sub.html#torch.Tensor.sub)
[`sub_()`](generated/torch.Tensor.sub_.html#torch.Tensor.sub_)
[`t()`](generated/torch.Tensor.t.html#torch.Tensor.t)
[`t_()`](generated/torch.Tensor.t_.html#torch.Tensor.t_)
[`transpose()`](generated/torch.Tensor.transpose.html#torch.Tensor.transpose)
[`transpose_()`](generated/torch.Tensor.transpose_.html#torch.Tensor.transpose_)
[`zero_()`](generated/torch.Tensor.zero_.html#torch.Tensor.zero_)

### Torch functions specific to sparse Tensors

| [`sparse_coo_tensor`](generated/torch.sparse_coo_tensor.html#torch.sparse_coo_tensor) | Constructs a sparse tensor in COO(rdinate) format with specified values at the given `indices`. |
| --- | --- |
| [`sparse_csr_tensor`](generated/torch.sparse_csr_tensor.html#torch.sparse_csr_tensor) | Constructs a sparse tensor in CSR (Compressed Sparse Row) with specified values at the given `crow_indices` and `col_indices`. |
| [`sparse_csc_tensor`](generated/torch.sparse_csc_tensor.html#torch.sparse_csc_tensor) | Constructs a sparse tensor in CSC (Compressed Sparse Column) with specified values at the given `ccol_indices` and `row_indices`. |
| [`sparse_bsr_tensor`](generated/torch.sparse_bsr_tensor.html#torch.sparse_bsr_tensor) | Constructs a sparse tensor in BSR (Block Compressed Sparse Row)) with specified 2-dimensional blocks at the given `crow_indices` and `col_indices`. |
| [`sparse_bsc_tensor`](generated/torch.sparse_bsc_tensor.html#torch.sparse_bsc_tensor) | Constructs a sparse tensor in BSC (Block Compressed Sparse Column)) with specified 2-dimensional blocks at the given `ccol_indices` and `row_indices`. |
| [`sparse_compressed_tensor`](generated/torch.sparse_compressed_tensor.html#torch.sparse_compressed_tensor) | Constructs a sparse tensor in Compressed Sparse format - CSR, CSC, BSR, or BSC - with specified values at the given `compressed_indices` and `plain_indices`. |
| [`sparse.sum`](generated/torch.sparse.sum.html#torch.sparse.sum) | Return the sum of each row of the given sparse tensor. |
| [`sparse.addmm`](generated/torch.sparse.addmm.html#torch.sparse.addmm) | This function does exact same thing as [`torch.addmm()`](generated/torch.addmm.html#torch.addmm) in the forward, except that it supports backward for sparse COO and CSR matrix `mat1`. |
| [`sparse.sampled_addmm`](generated/torch.sparse.sampled_addmm.html#torch.sparse.sampled_addmm) | Performs a matrix multiplication of the dense matrices `mat1` and `mat2` at the locations specified by the sparsity pattern of `input`. |
| [`sparse.mm`](generated/torch.sparse.mm.html#torch.sparse.mm) | Performs a matrix multiplication of the sparse matrix `mat1` |
| [`sspaddmm`](generated/torch.sspaddmm.html#torch.sspaddmm) | Matrix multiplies a sparse tensor `mat1` with a dense tensor `mat2`, then adds the sparse tensor `input` to the result. |
| [`hspmm`](generated/torch.hspmm.html#torch.hspmm) | Performs a matrix multiplication of a sparse COO matrix `mat1` and a strided matrix `mat2`. |
| [`smm`](generated/torch.smm.html#torch.smm) | Performs a matrix multiplication of the sparse matrix `input` with the dense matrix `mat`. |
| [`sparse.softmax`](generated/torch.sparse.softmax.html#torch.sparse.softmax) | Applies a softmax function. |
| [`sparse.spsolve`](generated/torch.sparse.spsolve.html#torch.sparse.spsolve) | Computes the solution of a square system of linear equations with a unique solution. |
| [`sparse.log_softmax`](generated/torch.sparse.log_softmax.html#torch.sparse.log_softmax) | Applies a softmax function followed by logarithm. |
| [`sparse.spdiags`](generated/torch.sparse.spdiags.html#torch.sparse.spdiags) | Creates a sparse 2D tensor by placing the values from rows of `diagonals` along specified diagonals of the output |

### Other functions

The following [`torch`](torch.html#module-torch) functions support sparse tensors:

[`cat()`](generated/torch.cat.html#torch.cat)
[`dstack()`](generated/torch.dstack.html#torch.dstack)
[`empty()`](generated/torch.empty.html#torch.empty)
[`empty_like()`](generated/torch.empty_like.html#torch.empty_like)
[`hstack()`](generated/torch.hstack.html#torch.hstack)
[`index_select()`](generated/torch.index_select.html#torch.index_select)
[`is_complex()`](generated/torch.is_complex.html#torch.is_complex)
[`is_floating_point()`](generated/torch.is_floating_point.html#torch.is_floating_point)
[`is_nonzero()`](generated/torch.is_nonzero.html#torch.is_nonzero)
...
[`is_tensor()`](generated/torch.is_tensor.html#torch.is_tensor)
[`lobpcg()`](generated/torch.lobpcg.html#torch.lobpcg)
[`mm()`](generated/torch.mm.html#torch.mm)
[`native_norm()`](generated/torch.native_norm.html#torch.native_norm)
[`pca_lowrank()`](generated/torch.pca_lowrank.html#torch.pca_lowrank)
[`select()`](generated/torch.select.html#torch.select)
[`stack()`](generated/torch.stack.html#torch.stack)
[`svd_lowrank()`](generated/torch.svd_lowrank.html#torch.svd_lowrank)
[`unsqueeze()`](generated/torch.unsqueeze.html#torch.unsqueeze)
[`vstack()`](generated/torch.vstack.html#torch.vstack)
[`zeros()`](generated/torch.zeros.html#torch.zeros)
[`zeros_like()`](generated/torch.zeros_like.html#torch.zeros_like)

To manage checking sparse tensor invariants, see:

| [`sparse.check_sparse_tensor_invariants`](generated/torch.sparse.check_sparse_tensor_invariants.html#torch.sparse.check_sparse_tensor_invariants) | A tool to control checking sparse tensor invariants. |
| --- | --- |

To use sparse tensors with [`gradcheck()`](autograd.html#module-torch.autograd.gradcheck) function,
see:

| [`sparse.as_sparse_gradcheck`](generated/torch.sparse.as_sparse_gradcheck.html#torch.sparse.as_sparse_gradcheck) | Decorate function, to extend gradcheck for sparse tensors. |
| --- | --- |
...
[`abs()`](generated/torch.abs.html#torch.abs)
[`asin()`](generated/torch.asin.html#torch.asin)
[`asinh()`](generated/torch.asinh.html#torch.asinh)
[`atan()`](generated/torch.atan.html#torch.atan)
[`atanh()`](generated/torch.atanh.html#torch.atanh)
[`ceil()`](generated/torch.ceil.html#torch.ceil)
[`conj_physical()`](generated/torch.conj_physical.html#torch.conj_physical)
[`floor()`](generated/torch.floor.html#torch.floor)
[`log1p()`](generated/torch.log1p.html#torch.log1p)
[`neg()`](generated/torch.neg.html#torch.neg)
[`round()`](generated/torch.round.html#torch.round)
[`sin()`](generated/torch.sin.html#torch.sin)
[`sinh()`](generated/torch.sinh.html#torch.sinh)
[`sign()`](generated/torch.sign.html#torch.sign)
[`sgn()`](generated/torch.sgn.html#torch.sgn)
[`signbit()`](generated/torch.signbit.html#torch.signbit)
[`tan()`](generated/torch.tan.html#torch.tan)
[`tanh()`](generated/torch.tanh.html#torch.tanh)
[`trunc()`](generated/torch.trunc.html#torch.trunc)
[`expm1()`](generated/torch.expm1.html#torch.expm1)
[`sqrt()`](generated/torch.sqrt.html#torch.sqrt)
[`angle()`](generated/torch.angle.html#torch.angle)
[`isinf()`](generated/torch.isinf.html#torch.isinf)
[`isposinf()`](generated/torch.isposinf.html#torch.isposinf)
[`isneginf()`](generated/torch.isneginf.html#torch.isneginf)
[`isnan()`](generated/torch.isnan.html#torch.isnan)
[`erf()`](generated/torch.erf.html#torch.erf)
[`erfinv()`](generated/torch.erfinv.html#torch.erfinv)