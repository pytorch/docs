...
torch.testing.assert_close(*actual*, *expected*, ***, *allow_subclasses=True*, *rtol=None*, *atol=None*, *equal_nan=False*, *check_device=True*, *check_dtype=True*, *check_layout=True*, *check_stride=False*, *msg=None*)[[source]](https://github.com/pytorch/pytorch/blob/2c911a1e1af237cf87c0e6e42a0fc9589043282b/torch/testing/_comparison.py#L1357)
...
- [`device`](generated/torch.Tensor.device.html#torch.Tensor.device) (if `check_device` is `True`),
...
[`qscheme()`](generated/torch.Tensor.qscheme.html#torch.Tensor.qscheme) and the result of [`dequantize()`](generated/torch.Tensor.dequantize.html#torch.Tensor.dequantize) is close according to the
definition above.

`actual` and `expected` can be [`Tensor`](tensors.html#torch.Tensor)'s or any tensor-or-scalar-likes from which
[`torch.Tensor`](tensors.html#torch.Tensor)'s can be constructed with [`torch.as_tensor()`](generated/torch.as_tensor.html#torch.as_tensor). Except for Python scalars the input types
have to be directly related. In addition, `actual` and `expected` can be [`Sequence`](https://docs.python.org/3/library/collections.abc.html#collections.abc.Sequence)'s
or [`Mapping`](https://docs.python.org/3/library/collections.abc.html#collections.abc.Mapping)'s in which case they are considered close if their structure matches and all
...
to [`partial()`](https://docs.python.org/3/library/functools.html#functools.partial) it to fit their use case. For example, if an equality check is needed, one might
...
torch.testing.make_tensor(**shape*, *dtype*, *device*, *low=None*, *high=None*, *requires_grad=False*, *noncontiguous=False*, *exclude_zero=False*, *memory_format=None*)[[source]](https://github.com/pytorch/pytorch/blob/2c911a1e1af237cf87c0e6e42a0fc9589043282b/torch/testing/_creation.py#L45)
...
type. Default `False`.
- **memory_format** (*Optional**[*[*torch.memory_format*](tensor_attributes.html#torch.memory_format)*]*) - The memory format of the returned tensor. Mutually exclusive
with `noncontiguous`.
...
torch.testing.assert_allclose(*actual*, *expected*, *rtol=None*, *atol=None*, *equal_nan=True*, *msg=''*)[[source]](https://github.com/pytorch/pytorch/blob/2c911a1e1af237cf87c0e6e42a0fc9589043282b/torch/testing/_comparison.py#L1634)

Warning

`torch.testing.assert_allclose()` is deprecated since `1.12` and will be removed in a future release.
Please use `torch.testing.assert_close()` instead. You can find detailed upgrade instructions
[here](https://github.com/pytorch/pytorch/issues/61844).