...
| TorchInductor Tracing | Show time taken in each TorchInductor stage + output code and graph visualization | set the environment variable TORCH_COMPILE_DEBUG=1 or `torch._inductor.config.trace.enabled = True` |
...
Minifying Backend Compiler Errors). If the error still
...
then Minifying TorchInductor Errors.
...
There are [different choices](user_guide/torch_compiler/torch.compiler.html)
...
Minifying TorchInductor Errors with one important
...
Minifying TorchInductor Errors is that the minifier is
...
able to introduce necessary guards (see [Making Dynamo Sound: Guards](user_guide/torch_compiler/torch.compiler_dynamo_deepdive.html#making-dynamo-sound-guards))
