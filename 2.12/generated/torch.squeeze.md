...
(A×1×B×C×1×D)(A \times 1 \times B \times C \times 1 \times D)(A×1×B×C×1×D) then the input.squeeze()
will be of shape: (A×B×C×D)(A \times B \times C \times D)(A×B×C×D).

When `dim` is given, a squeeze operation is done only in the given
dimension(s). If input is of shape: (A×1×B)(A \times 1 \times B)(A×1×B),
...
If the tensor has a batch dimension of size 1, then squeeze(input)
