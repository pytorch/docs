...
[`torch.autocast`](../amp.html#torch.autocast) and `torch.amp.GradScaler` together.

Instances of [`torch.autocast`](../amp.html#torch.autocast) enable autocasting for chosen regions.
...
[`torch.autocast`](../amp.html#torch.autocast) and `torch.amp.GradScaler` are modular.
...
call `unscale_` just before `step`, after all (scaled) grads for the upcoming
`step` have been accumulated. Also, only call `update` at the end of iterations
...
device, like [`torch.nn.DataParallel`](../generated/torch.nn.DataParallel.html#torch.nn.DataParallel). The fix is the same:
...
[`torch.amp.custom_fwd()`](../amp.html#torch.amp.custom_fwd) and [`torch.amp.custom_bwd()`](../amp.html#torch.amp.custom_bwd) decorators as shown in
the relevant case below.

### Functions with multiple inputs or autocastable ops

Apply [`custom_fwd`](../amp.html#torch.amp.custom_fwd) and [`custom_bwd`](../amp.html#torch.amp.custom_bwd) (with no arguments) to `forward` and
...
Apply [`custom_fwd(device_type='cuda', cast_inputs=torch.float32)`](../amp.html#torch.amp.custom_fwd) to `forward`
